...
# 個股上反覆低買高賣」(見 docs/13 §0、docs/27)。
#
# **2026-09-08:方向 battery 已在修好的資料上跑完並通過,面板留著,而這一條照樣
# 留著。** 這兩件事沒有矛盾,因為**面板宣稱的不是這個名字宣稱的東西**:battery
# 立起來的是「一個**群體層級**的傾向會延續到下一段時間」(存活 pair 的
# exceeds-both 57.4%,對上約 22–24% 的機率值;obs/exp 2.40 對安慰劑 0.91),而
# 「關鍵分點」宣稱的是一個**身分**——某個分點**就是**這種分點。那個身分自己
# 重新賺回來的比率是 2.0%。面板呈現計數,名字則指認一個人物,後者資料不支持。
# 所以:**不要把「battery 通過」讀成這個名字自動解禁**。要解禁必須另外有一份
# 量測直接證明「同一對再次達標」這件事本身,並附一份具名的決策紀錄。在那之前,
# 任何使用者看得到的字串出現這四個字都是錯的。
BANNED_WORDS = [
