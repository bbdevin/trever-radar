...
觀察 vs 重建:怎麼分辨,以及為什麼不加欄位
------------------------------------------
``computed_at`` **就是** provenance 訊號,不需要第二個欄位:

    一列是「觀察」若且唯若 computed_at 的日期是 as_of 的次日曆日。

那是 00:05 夜間作業的執行時刻(``crontab.example``),其餘一律是事後重建。
要問「某個日期 D 當時知道什麼」,就篩 ``computed_at <= D+1``;匯出端若需要一個
旗標,由這條規則推導出來,**不要存**。

不加欄位是刻意的。``schema.py`` 已把 ``computed_at`` 定義為「資料可得性」,
再加一個 ``is_reconstructed`` 等於把同一件事存兩份,而兩份會漂。也**不要**
拿 ``DEFINITIONS_VERSION`` 兼差:它是給「定義變了、舊列仍可辨識」用的,
把 provenance 塞進去會破壞那個契約。

重建不等於失真(2026-09-14,Fable 5.1 決策)。反直覺但關鍵:
本模組的窗口是 ``DEFAULT_WINDOW_DAYS = 60``,所以 as_of 2026-09-01 讀的是
2026 年 6–9 月——而 490 日回補補的是 2024-08～2025-03,**落在窗口外**,對這
幾列毫無影響。真正落在窗口裡的是 2026-07-16～08-19 那段自家抓取破洞:一列
**當時**寫的 09-01 會帶著那個破洞造成的膨脹 unknown 計數;修好之後重算的那
一列,用的是 09-01 當天就公開可得的資料。``date <= :as_of`` 的上界保證沒有
未來資訊。**point-in-time 承諾的是市場資訊,不是我們的停機。**
