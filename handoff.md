# Handoff — 2026-08-27（E2 唯讀 shadow 程式完成，正式 VPS 未執行）
...
- **Done**: S4 V2 兩階段、S4 phase JSON／首頁標示；Armed A1 匯出契約；A2 綜合榜、S12 fail-closed 與 versioned strategy lifecycle contract；docs/37 B+C+D 公司資訊、題材 freshness 與集團鑽取；E2 唯讀 shadow contract。E2 targeted pytest **11 passed**、E2＋既有分點 targeted **39 passed**、完整 pytest **247 passed、55 subtests**；前端最近一次 `npx tsc --noEmit` 與乾淨快取 `npm run build`（12/12 static pages、2/2 export）均通過。repo 沒有 `npm run typecheck` script，勿把該指令記為驗收結果。非阻塞環境債仍是 Node 20 未來不受 `@supabase/supabase-js` 支援。**未正式 VPS import/export、未正式 DB 回算，E2 未跑正式 DB**。
...
- B 公司地址／股務代理、C 題材 lifecycle 與 D 集團 mapping 的程式／fixture／UI 已完成，正式 VPS `import-themes`／`import-geo`／`export-json` 尚未執行；E1 庫藏股官方來源與 KB1 仍為分期規劃。E2 point-in-time shadow CLI／JSON contract（獨立 buy／sell，無配對）已完成且測試通過；schema／歷史回算仍需人工確認。
- **KB2 `BUYBACK_BRANCH` 明確不實作**；E2 **不做交易獲利歸因**。不得將舊版 KB2 規劃復活成推測徽章。
- 下一位 Executor 先讀 `docs/37` §9；A2、B、C、D 與 E2 shadow code 已完成，接續 E1 KB1 或等待人類確認 E2 schema／歷史回算。B/C/D 的正式 VPS import/export 仍不得自行執行；不得改 workflow、VPS 排程、正式 DB 或執行全市場回灌。
