# Handoff — 2026-08-27（A2 + docs/37 B+D 程式完成，正式 VPS 未執行）
...
B+D 已完成程式/fixture/UI：`company_profiles` additive 官方公司欄位、舊 SQLite runtime additive migration、個股 `industry/company_profile` safe fallback；官方華新麗華集團 mapping (`1605/2344/2492/5469/6116`)→`groups.json`/個股 `company_groups`→次要 `/group?id=`。群組摘要不依賴 radar pool。正式 VPS `import-geo`/`export-json`、正式 DB migration/回算、排程/workflow/secrets 均未執行；不可自行執行。
...
- **Done**: S4 V2 兩階段、S4 phase JSON／首頁標示；Armed A1 匯出契約；A2 綜合榜、S12 fail-closed 與 versioned strategy lifecycle contract；docs/37 B+D 公司資訊與集團鑽取。B+D review 補強的 targeted pytest **30 passed**；完整 pytest **228 passed、55 subtests**；`npx tsc --noEmit` 成功；清除兩份失敗的本機 `.next` 快取後，`npm run build` 完整通過（12/12 static pages、2/2 export）。兩個既有盤中 worker regression tests 已固定交易時段，消除依執行時鐘的假失敗，production worker 未改。非阻塞環境債：`@supabase/supabase-js` 已提示未來將停止支援 Node 20，後續本機／CI 應規劃升至 Node 22。**未正式 VPS import/export、未正式 DB 回算**。
...
- B 公司地址／股務代理與 D 集團 mapping 的程式／fixture／UI 已完成，正式 VPS `import-geo`／`export-json` 尚未執行；C 題材分類與近期熱度、E1 庫藏股官方來源與 KB1 仍為分期規劃。E2 為 point-in-time shadow 統計，需人工確認 schema／歷史回算。
- **KB2 `BUYBACK_BRANCH` 明確不實作**；E2 **不做交易獲利歸因**。不得將舊版 KB2 規劃復活成推測徽章。
- 下一位 Executor 先讀 `docs/37` §9；A2、B、D code 已完成，接續 C 題材 freshness → E1 KB1；E2 先做唯讀 shadow。B/D 只剩正式 VPS import/export，仍不得自行執行；不得改 workflow、VPS 排程、正式 DB 或執行全市場回灌。
