...
function BranchPanel({ data }: { data: StockJson }) {
  ...
  return (
    <div className="grid gap-3 overflow-x-auto rounded-[var(--r-lg)] border border-border bg-card p-3.5 shadow-[var(--shadow-card)]">
      <div className="grid grid-cols-2 gap-2.5 md:grid-cols-[1.1fr_repeat(3,1fr)]">
...
function BranchRow({ b, expanded, onToggle }: { b: { name: string; net: number; history?: { t: string; net: number }[] }; expanded: boolean; onToggle: () => void }) {
  ...
  return (
    <div className="overflow-hidden rounded-[var(--r-sm)] border border-border bg-card transition-[box-shadow,border-color] hover:border-[color:var(--border-strong)] hover:shadow-[0_2px_6px_rgba(0,0,0,0.2)]">
      <div className="flex cursor-pointer items-baseline justify-between px-2.5 py-2 text-[12.5px] select-none" onClick={onToggle}>
