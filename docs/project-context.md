...
- `pipeline/radar/compute/*` — 指標與評分(純函式,golden-file 測試);技術指標已在 `compute/indicators.py`
...
00 藍圖總表|01 產品定位|02 版本範圍與驗收|03 資料源評估|04 全部評分規則|05 資料表|06 架構(部署章節已被 12 取代)|07 前端頁面|08 排程流程|09 籌碼K線|10 資安法規|11 AI 開發流程|**12 零成本修訂(現行架構)**|**STATUS(目前進度,單一真相)**|../DEPLOY(部署)

## 目前狀態(2026-07-07,細節看 STATUS.md)

已上線 https://radar.techtrever.com,GitHub Actions 每交易日自動更新,`main` push 也會正式部署。管線(日K/權證主檔/權證彙總/法人/融資券/產業別/雙軌歷史回補/還原因子 CLI/技術指標)與前端(族群資金流+熱門/爆量/強勢/權證四榜+個股 K 線+MA疊線+技術摘要+個股權證 Tab)完成。**下一步:評分模組(04;完整權證分/技術分/法人融資分/題材分與 reasons/risks)**。
...
8. 現在的權證榜是異動排序雛形:`call_turnover / 20日均值`,不是最終 04 §1 的 0–100 權證分;不要把它當正式分數。
9. 技術指標必須用還原價:`adj_close = close * adj_factor`;`adj_factor` 由 `python -m radar compute-adjustments --ids/--top/--all` 補,尚未全市場自動排程。
