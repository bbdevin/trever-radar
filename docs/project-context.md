...
17. 2026-08-20 品牌+PWA(`e3cefcd` 身分 + 後續完善):**TR Monogram + Radar + 隱性 Wheel**;Header 用 `trever-radar-mark.svg`;manifest=`web/app/manifest.ts`。**不要重做 Logo、不要蓋掉 Header mark、不要因 `design-system/stock/MASTER.md` 舊綠/`#22C55E`/Fira 把現站改回去。** PWA 先於原生;SW 只 cache app shell,`/data` Network Only + `no-store`,禁止把過期訊號當最新。不重寫 React Native。
