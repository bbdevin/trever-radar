...
| 順序 | 文件 | 內容 | 狀態 |
|---|---|---|---|
| 1 | `21` | Access 鎖站 A0–A2 | ✅ 已完成(Cloudflare 設定) |
...
| F1 | ✅ 完成(2026-07-12) | F1.1/F1.2 自選距關鍵價%+排序(合入 IA-4A);**F1.3 一鍵加入今日 Armed ✅(2026-07-12)**:`/watchlist` 頂部動作區「加入今日 Armed(N 檔)」,N=`radar.lists.armed` 中尚未在自選者;只加不減(pending 先過濾已在自選者,逐檔 `toggle` 新增、失敗不中斷),完成 Sonner 回饋「已加入 X 檔(;失敗 Y 檔)」;未登入/今日無 Armed/全部已在自選 三態 disabled(`lists.armed` 空防禦處理);`aria-busy`;不自動同步。`npm run build` 過 |
| F2 | ✅ 完成(2026-07-11) | 可與 Armed A1 同波評估 |
| F3 | ✅ 完成(2026-07-12) | 個股頁訊號摘要面板(合入 IA-2) |
| F4 | ◑ 部分完成 | **F4.1 掃描收斂 ✅**:hot/surge/strong/weak 收成「市場掃描」一級 Tab + sub-selector(由 IA-1B 完成,2026-07-12)。**F4.2 策略四類分群 ✅(2026-07-12)**:13 策略 pills 依 docs/20 §4.1 分「籌碼事件/突破發動/趨勢續強·回踩/低檔反轉」四組;組標題含總檔數 badge + chevron + `aria-expanded`;預設只展開籌碼事件(session 內、不持久化);選中策略落在收合組時自動展開;預設選中改籌碼事件組第一個有檔數者(皆無則 S11);未改 S code 語意。`npm run build` 過 |
