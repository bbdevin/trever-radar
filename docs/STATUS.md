...
## 2026-09-26 驗收 22:00 評分刷新;首頁摘要兩句永久性的假話

- [x] **22:00 評分刷新在正式機生效**。09-24 22:00 那輪走了刷新路徑(`skip compute-branch-stats：今天 17:40 已上線`),23:05 收尾;09-24 的紀錄之列**缺分點 0 檔、缺法人 0 檔**(修前 09-22 / 09-23 為 364 / 227)。綜合榜 1 檔:2636 台驊控股 68 分,六個分項都有資料——第一次有「資料完整的上榜」。16:10 那版正確扣留(739 檔全缺分點),摘要說「尚未到齊」。09-25 沒有評分列是**中秋節休市**,不是故障;當天各輪重算的都是 09-24。
- 🚨 **摘要第三句「權證、題材分類資料尚未更新,稍後自動補齊」是兩句永久性的假話,修掉**。
  - **題材**:資料集層級用的是「任一個題材不是 active 就 stale」,而 `theme_lifecycle` 刻意只降級、不自動退休——來源下架過一個題材,題材分類就**永遠** stale(正式站 831 active、3 stale)。改成問「每週一的匯入有沒有跟上」:沒有任何 active,或最新一次匯入距資料日 ≥ 7 天才算。個別題材的 lifecycle 不動,下架的照舊進不了熱門題材。
  - **權證**:`partial_stale` 數的是「最後一列早於今天」的全部標的,而權證全數到期的標的從此不再有列(09-24 有 31 檔,最後一列散在 07-16..09-08)——天天 stale。`warrant_stock_daily` 每天對每檔有權證的標的都寫一列(零成交也寫,每天約 846 列),所以批次缺漏的訊號是「**前一個交易日有列、今天沒有**」,只數那一種。
  - 兩者的共同形狀與 09-21 期貨那一條相同(`f38fe50`):一個**結構上**永遠為真的 stale,配上一句「稍後自動補齊」的承諾。
- [x] 新增 6 條測試,在舊規則下都會紅(舊規則把到期標的、下架題材算進去,7 天前的匯入反而不算)。pytest 955 passed / 3875 subtests。

## 2026-09-24 整站掛載圖:每個畫面元件都要從路由入口真的被畫出來(自動發現)

- [x] **`pipeline/tests/test_components_are_mounted.py`**,上一輪記下的「下一張票」。這週兩次撞見「功能記成已出貨、從沒出現在任何一頁、測試全綠」(`a0c7b06` 期貨切片、`a5ae269` 字級按鈕);期貨那份用**手抄清單**守住了六個元件,但手抄清單守不到下一個——新元件沒人記得登記。這份改成**自動發現**:從 9 個路由入口(`app/**/page.tsx`、`layout.tsx`)的 default export 出發,沿「import 進來而且在某個元件函式本體裡寫了 `<Name`」的邊走,走不到的元件就紅燈。**邊的起點是元件,不是檔案**:同檔裡只被死元件畫出來的元件不算掛上;helper 函式(`renderRows()`)也是節點。
- 📌 **現況:87 個功能元件全部走得到,沒有新的漏網之魚**。只有「庫」沒被整組用到:`components/ui/*`(shadcn,Badge、Card、Separator、Tabs 等未用 primitive)與 `components/Icons.tsx` 的 7 個圖示——兩者刻意豁免,理由寫在模組說明。
- 📌 **解析器第一版自己出了兩個假紅燈,修掉才算數**:全大寫常數(`CREDIBILITY_TOOLTIP`)被當成元件、以及 `const X = forwardRef(...)` + 檔尾 `export default X;` 的寫法(`BranchFlowSection`)沒被認成 default export。後者已釘進非空性檢查。
- [x] 四個「應該變紅」的構造案例全部真的變紅:拿掉唯一呼叫點(a5ae269 的形狀)、註解掉呼叫點、只被同檔死元件畫出來、import 還在但 JSX 拿掉。不執行任何東西(不需要 node / Next build),與既有的腳本結構測試同一種手法。
- ⚠️ **仍然守不到的,如實記下**:條件永遠為假的呼叫點(`{false && <X />}`)、以值傳遞的元件(`component={X}`,目前全站沒有)、以及資料結構上不可能渲染(a0c7b06 那一種——元件有掛上,但它等的資料永遠不會來)。最後一種要靠 payload 形狀的測試,不是掛載圖。

## 2026-09-24 綜合榜的假上榜:當日評分被凍結在缺分點的版本(我造成的迴歸),修掉並加資料齊全閘門

- 🚨 **根因是 09-17 那次 22:00 改純匯入(`BRANCH_ROUND_MODE=import`)——我提的、使用者核准的,代價當時沒看全**。17:40 那輪匯入分點常常還不完整(09-22 涵蓋 983/1964、09-23 1441/1958),照樣跑 `compute-scores` 並寫完成標記;22:00 那輪補齊了(1964、1958),但改成純匯入後**不再重算評分**;00:05 夜間作業看到標記就略過。而 `compute-scores` 只寫最新一個價格日,**當天那一列隔天就再也不會被重算**——所以當日評分永久凍結在缺分點的版本。當時記下的代價是「22:00 才進來的列要等隔天 17:40 才進 `stock_stats`」,沒有人注意到 `daily_scores` 根本不會回頭補。
- 📌 **量到的**:缺分點的評分列 09-10..09-17 每天 0 檔;改動生效後 09-18 / 09-22 / 09-23 = **363 / 364 / 227 檔**(09-21 為 0,當天 17:40 匯入剛好完整)。09-23 那 227 檔,09-22 全部都有分點列,而且它們 09-23 的分點**現在都已在 DB**——資料在,只是沒人重算。
- 📌 **為什麼這會製造假上榜**:分項缺值時 `combine()` 把權重重分給其他分項,而題材分平均偏高。`daily_scores` 全史 `final ≥ 65` 的 23 列裡,**15 列分點與法人皆缺、2 列只缺分點、只有 6 列資料完整**(實測,不是推論)。09-24 14:16 那一版 506 列全缺分點與法人,卻有 7 檔 ≥ 65。另一個結構性上限:分點分的 B4/B5/B7 從未實作,實作滿分 70(`docs/04` 以 100 設計),資料完整時 42 天只有 6 筆 ≥ 65、48 筆 ≥ 60、201 筆 ≥ 55——**門檻要不要動是使用者的決定,本輪不動**。
- 📌 **資料停擺記錄**:`import_logs` 在 08-10 15:09 到 08-14 14:15 之間沒有任何列,對應 08-11..08-13 三天沒有評分;08-10 的紀錄之列是 14:16 那一版初稿(缺分點與法人,那天 6 檔上榜全是它)。07-07..07-09、07-13、07-14 另外 5 天沒有評分,在 07-18 VPS 接手之前,查不到原因。
- [x] **修法一:22:00 那輪在今天已上線時仍重算當日評分**(`vps/scripts/daily-branches.sh`)。`SCORES_REFRESH=1` 只跳過 `compute-branch-stats`(74 分鐘、差 0.011%,理由不變),其餘 `compute-scores` → `compute-performance` → `export-json` → `prune` → `deploy` 照同一條鏈走,**不重寫完成標記**(標記的內容是第一次上線的時刻)。`compute-scores` 只讀原始表,不依賴分點統計。失敗通知宣告自己的後果:「當日評分維持缺晚到分點的版本;夜間作業不會補算評分」。預期每晚多約 20 分鐘(export 約 19 分鐘 + deploy 約 2 分鐘),約 23:10 收尾。**不動 crontab**。以 stub 在 WSL 實跑三種情境(import+標記、import+無標記、full)確認行為;實跑時抓到自己一個瑕疵——模式判斷處先發了一則「重新上線」的成功通知,那時什麼都還沒做——改成只寫 log,並加測試鎖住。
- [x] **修法二:export 的資料齊全閘門**(`json_export.score_list_gate`,使用者核准)。缺分點**或**缺法人的評分列 × 10 > 評分列數 → 綜合榜不排名(`lists.score = []`),新增平行鍵 `score_list_meta {min_final, scored, missing_branch, missing_inst, withheld, max_final}`(全是整數與一個布林)。資料齊全時這兩者的缺值基準都是 0(09-08..09-17 實測),所以一成不會擋正常日子;權證不列入(約兩成股票本來就沒有權證)。摘要句在扣留時說「尚未到齊」,**不說「暫無達門檻」**——那是把「不知道」講成「知道沒有」。這道閘門不改 `final`、不改門檻,只決定今天排不排名。
- [x] **修法三:空榜的句子分兩種**(`web/lib/scoreList.ts`,使用者核准)。扣留 →「分點/法人資料尚未到齊,暫不排名」附缺幾檔,**不附最高分**(那個分數算在缺資料的列上);資料齊全而沒人達標 →「今日沒有任何一檔達 65 分,今日最高 N 分」。門檻從 payload 讀。缺 `score_list_meta`(舊 payload + 新程式碼)→ 沿用原句,不猜。**不列出未達門檻的清單**——那等於在畫面上偷偷降門檻。
- ⚠️ **不回補歷史**:09-18/22/23 已凍結的評分列不重算。`docs/04` §12 的前瞻記錄法要求列凍結,而且重算歷史列屬於正式 DB 重算,須另外核准。這三天的綜合榜紀錄因此仍含缺資料的列,記在這裡。
- ⚠️ **對「隔天大漲」的連動**:依 `docs/39` §3.8,本閘門改變了「誰算上榜」,v1 的計數即便重跑也要從今天重新起算;v1 過去 23 筆有 17 筆是缺資料的假上榜,樣本本身不是一條乾淨的規則。依 Opus 5.5 的建議**擱置**,等閘門上線累積 60 個市場日後再評估要不要寫 v2(在那之前任何人不看往後報酬分布)。
- [x] **commit 前過獨立驗證**(驗證者用**真的** `lib.sh` 實跑四種情境、做變異測試):功能主張全部成立、今晚 22:00 無會出事的 bug;另抓到五處,全部修掉。①刷新失敗的後果句寫「網站仍是 17:40 的內容」——錯,21:20 資券輪也跑 compute-scores 與 deploy,網站是 21:20 的內容。②同一模式下匯入階段就失敗時,沿用了「00:05 夜間作業會重算」——有標記時夜間作業整夜略過,這句不成立(舊碼就有)。兩者改成開跑時就依「import + 有標記」宣告同一句正確後果。③變異「deploy 前插一行 `if [ "$SCORES_REFRESH" = 1 ]; then exit 0; fi`」通過整份測試(會靜靜不上線、回 0、不通知)——新增測試鎖定旗標只准被讀兩次,已確認這個變異被殺。④一列評分都沒有時閘門判成「資料齊全」,前端就會說「今日沒有任何一檔達 65」——改判為扣留,前端講「今日評分尚未產生」。⑤ 65 有兩份(常數與上榜那一行的字面值)——上榜那一行改讀 `SCORE_LIST_MIN_FINAL`;另外 docs/39 battery 的絆線不再證明「battery 與 export 是同一張榜」(export 多了閘門),新增一條測試把這個**刻意的**分歧寫在明處:v1 的上榜定義在閘門出現前就凍結,不得事後接上;任何 v2 必須含閘門。
- [x] pytest 942 passed / 3875 subtests;`node --test` scoreList + futures + freshness 60 passed;`npm run build` 成功;`tsc --noEmit` 只剩既有的 TS5097 形狀(新測試檔同形狀多一個)。

## 2026-09-24 「隔天大漲」battery 第 1 次執行:**DO NOT SHIP(UNDERPOWERED)**——而真正的發現是綜合榜 49 天裡有 40 天是空的

- 📌 **依 `docs/39` §3.5 的紀錄義務,逐項寫下**。事前登記 `f8a57b5`;battery 程式 `193c812`;**第 1 次執行**,2026-09-24 14:05 於 VPS 以獨立 clone、`mode=ro` 執行;`as_of` = **2026-09-23**(執行當下 `daily_prices` 最大日,由 `sqlite3 -readonly` 讀出、程式強制檢查,不由人挑;兩市場當日列數 1,380 / 1,015,完整)。`daily_scores` 2026-07-06..2026-09-23、49 個日期、32,745 列;評估期 57 個市場日(H1 = 07-06..08-13 共 28 天,H2 = 08-14..09-23 共 29 天)。原始 JSON 進版控:`docs/evidence/next-day-surge-battery-20260924.json`。
- 📌 **n = 23、h_L = 0**(H1:9 事件 0 命中;H2:14 事件 0 命中)。事件否決全為 0;對照池否決 `R1_no_entry_day` 749(as_of 當天的列,不成熟)、`R1_no_entry_row` 6、`R1_bad_open` 1、R2 兩碼皆 0。`tie_at_cap` 0、`over_cap_days` 0。P_stock 十個 seed 的 h_P = 0,0,1,1,2,0,1,1,1,1;P_date = 0,1,0,0,0,1,2,0,1,2;σ_P 在 h_P = 2 時為 1.351,其餘觸底 1.0。每半 h_P:P_stock H1 0–1、H2 0–1;P_date H1 0–1、H2 0–1。沒有任何一組抽不滿。
- 📌 **裁決**:A 不過(n = 23 < 30、h_L = 0 < 30)→ **UNDERPOWERED,DO NOT SHIP**。B 與 C 也全部不過(h_L = 0,20 個判準與 40 個半段判準無一為正),但依 §3.5 裁決理由取 A:這是**檢定力不足的無結果,不是否證**。沒有任何鍵、表、面板被寫入。伴隨計數(只報告):`board_hits` 0、`entry_open_at_limit_proxy` 0、`all_days_events` 23(= 事件數,沒有任何一檔連續兩天上榜);依 `final` 區間 65–74 為 21 事件、75–84 為 2 事件、≥ 85 為 0。
- 🔍 **n = 23 先查證不是實作假象,才寫進來**。直接數 `daily_scores` 中 `final ≥ 65` 的列:**全表就是 23 列**,分布在 9 天、21 檔;`final` 全表最高值 78。與 battery 逐數相符。
- 🚨 **比裁決更重要的發現:綜合榜(`final ≥ 65`)在 49 個評分日裡有 40 天是空的**。有上榜的 9 天是 07-27、08-04、08-07、08-10(6 檔)、08-19(9 檔)、08-31、09-08、09-11、09-18(2 檔)。首頁那張「綜合榜」大多數日子什麼都沒有——這是一個產品現況,與本功能是否上線無關,之前沒有任何紀錄提過。另有一個小落差:評估期 57 個市場日,`daily_scores` 只有 49 個日期,有 8 個市場日沒有評分列(未查原因,記下)。
- ⚠️ **依 §3.7,下一次重跑最早要再累積 60 個新市場日(約 2026-12 中)——但老實講,以這個上榜頻率,A 在可見的未來不會過。** 57 個市場日產生 23 個事件;A 要的是 30 個**命中**,而對照組的命中率大約是 23 次裡 0–2 次。照這個速度,湊到 30 次命中需要的是以「年」計的資料,不是 60 天。這**不是**放寬 30 的理由(§3.7 明文禁止);它的意思是:在目前的評分規則下,「綜合榜隔天大漲次數」這個 v1 功能實際上做不出來。要不要改題目(v2 須另立事前登記、只能用 v2 commit 之後的交易日)是使用者的決定,不在本輪範圍。
- [x] **實作在上正式資料前先過獨立驗證**:驗證者確認實作與 `docs/39` 一致、無影響 n/h_L/h_P/裁決的 bug,但抓到 9 個「錯誤實作也能通過全部測試」的變異(e 讀儲存值、段落跨過缺評分日、拿掉 has_close、評估期起點、H1/H2 邊界、奇數日方向、C 放寬成 ≥ 0、B 任一 seed 過就過、R2 容差邊界),以及 `--as-of` 未強制檢查。全部補上呼叫點層級的測試,18 個變異全殺,`--as-of` 改為必須等於 `daily_prices` 最大日(`--historical` 只供重現舊裁決,並寫進報告)。pytest 931 passed / 3838 subtests。

## 2026-09-24 「隔天大漲」事前登記 commit(`f8a57b5`),battery 尚未實作、尚未執行

- **事前登記的 commit hash 是 `f8a57b5`**(`docs/39_next_day_surge_count_preregistration.md`)。**它在任何人對 `daily_scores` 的 `fwd_*` 欄位做過彙總、或把 `daily_scores` 與次日價格 JOIN 過之前落地**——這份文件的全部效力就在這個時間順序上,不在內容有多完備。使用者的原話是「幫我規劃功能 就是隔天大漲機率」,交付的是規劃本身,而規劃裡最重要的一條是**什麼結果代表這個功能不上線**。
- 📌 **三條裁決,都在看資料之前寫死**。①「大漲」= `100 × close(s, e) ≥ 107 × open(s, e)`:7% 借自 `docs/04` §13 `S1_REBOUND_RELAXED`(本專案唯一把「大漲」寫成數字的地方),「開盤到收盤」這一段來自 `docs/04` §12——兩個都不是從資料裡挑的。**不含前一日收盤到開盤的跳空**,因為那一段沒有人買得到:跳空 +3% 再鎖漲停的日子算**未命中**,這個代價事前承認,方向是命中變少。②「機率」→ `hits` / `events` 兩個整數,外加同一條規則在**沒有訊號的日子**上產生的計數(兩組對照、十組 seed 的整數 min–max)。N/M 就是使用者問的那個機率的最大概似估計;給 N 與 M 是讓分母可見,給基準是因為「96 / 1,240 算高還是低」只有跟沒訊號的日子比才有答案。③條件是**上榜**(`final ≥ 65`、前 40、連續日取首日)、**整體合併**——一檔上榜 3 次的股票沒有分母,所以不做個股數字。
- 📌 **不上線的條件**:A 檢定力(`n ≥ 30` **且** `h_L ≥ 30`)、B 資訊性(同股與同日兩組等量對照 × seed 0..9,**20 個判準全部**要 `h_L − h_P ≥ 2σ_P`)、C 跨半段一致性(兩半各自正號)。任一 NOT EVALUABLE 即不上線。過不了就是**整個功能不上線**,不是「上線但加註」——先例是 `docs/04` §2 的往後看方向(n=18、1.81σ,什麼都沒上線)與 `docs/38` 的期貨量異常(十組 seed 全過,上線了)。
- ⚠️ **第一次很可能是 UNDERPOWERED,而那是正常結果**。`compute-scores` 2026-07-07 才建立、只寫最新一天、從未回補。依 §4 步驟 2 記錄涵蓋度(這幾個數不是任何門檻的輸入):`daily_scores` = **2026-07-06 至 2026-09-23、49 個日期、32,745 列**。設計時估的是約 55 個市場日,實測 49,更少。文件在凍結規則裡就把這件事寫明,免得日後有人拿「樣本不夠」當放寬 30 的理由——不足時依 §3.7 累積 60 個新市場日才可重跑一次。
- 📌 **§9 是我加的,不是設計者寫的,目的是讓這份事前登記不必靠自稱**。commit 之前我跑過一次查詢,逐字記在 §9:`SELECT MIN(date), MAX(date), COUNT(DISTINCT date), COUNT(*) FROM daily_scores`。它沒碰 `fwd_*` 任一欄、沒 JOIN 價格、沒依 `final` 篩選,所以不可能影響門檻——但讀者不該需要相信我這句話。同一節順手修正我自己先前講太滿的一句:**本專案並沒有全面禁止輸出比率**,`strategy_meta` 就有 `win_rate`,配 `samples` 與 `_MIN_SAMPLES_20D = 30` 的閘門(`json_export.py:520,573`)。只出計數是 `docs/38` 的紀律,本功能依理由自願採用且更嚴。
- 📌 **重用儲存欄位的方式是「當對照,不當來源」**。`entry_date` 與 `fwd_1d` 只作完整性閘門(R2):`entry_date ≠ e(t)` 或 `|stored − recomputed| > 0.01` 就否決該事件並計數;統計量一律從 `daily_prices` 的**原始** open/close 重算(同一列兩個價,`adj_factor` 相消,所以這個統計量結構上與還原因子無關——2026-09-03 的事故正是還原因子錯了兩個月沒人發現)。`forward_returns` 會跳過缺列或開盤為 0 的日子往後找,所以缺了 t+1 時 `fwd_1d` 講的其實是 t+2,而列一旦有 `fwd_20d` 就不再重訪;對 20 日用途無害,對 1 日用途就是錯的。
- [x] ~~**下一步是 §4 步驟 3–5,尚未動工**~~(同日完成步驟 3–4,見上一條;步驟 5 因裁決不上線而不執行):唯讀的 `pipeline/radar/compute/next_day_surge_battery.py`(`LOW_SAMPLE_SURVIVORS` 從 `branch_window_direction_battery` import、`PLACEBO_SEEDS` 從 `futures_volume_battery` import,不複製第二份 30 與第二份 2σ)、跑一次、把 §3.5 的紀錄寫回本檔、**只有 PASS 才**加 `list_next_day_surge` 鍵。

## 2026-09-23 `compute-branch-stats` 實測 1,702 秒——我先前寫的「約 11 分鐘」錯了 2.5 倍,原因是取樣有偏

- 🚨 **更正一個我自己寫進本檔與 commit 訊息的預期**。09-18 那筆記錄說 binary search 之後這一步會落在「12–18 分鐘」,依據是 20 檔 profile 的 95.46s → 13.83s 外推。**實測 1,702 秒(28.4 分鐘)**,加速比 **2.7×** 而不是 profile 顯示的 6.9×。改動前是 4,465 / 4,627 秒,所以每天仍省下約 49 分鐘——但我宣稱的數字錯了 2.5 倍。
- 🔍 **原因是取樣方式,不是程式**。`prof.py` 把 `LIMIT 20` 接在一個以 `ORDER BY s.id` 結尾的查詢後面,取到的是**代號最小的 20 檔**——系統性偏差的樣本,不是隨機抽樣。那 20 檔的分點歷史長度與價格序列長度不能代表 1,988 檔的分布,而 `forward_returns` 的成本正好與這兩者相關。
- ⚠️ **當初的驗證者就標記過這個假設,我沒有照著它調整說法**:它寫「前提是那份 20 檔 profile 的百分比分布可以原封不動推廣到全規模正式環境——這個假設 commit 本身沒有獨立證成」。它是對的。這裡記下來不是為了自責,是因為**同一份 profile 也推出了「下一個槓桿是 `fetchall`」這個結論**,那個結論因此同樣未經證實,重新量之前不應拿來規劃。
- [x] **用固定種子隨機抽 100 / 1,991 檔重量,結論與有偏那份相反**。總 tottime 132.75s,**SQL 115.46s(87.0%)、Python 17.29s(13.0%)**;`fetchall` 58.66s(201 次)、`execute` 13.08s(281 次),Python 側的主角是 **SQLAlchemy 為批次 INSERT 逐列建 bind parameter**(`visit_bindparam` / `_create_bind_param` 各約 116 萬次,合計約 17s),不是分析邏輯——`forward_returns` 只剩 3.71s。有偏那份說 `forward_returns` 的 genexpr 佔 87%,這份說 SQL 佔 87%。
- ⚠️ **這一次不外推絕對時間,理由寫下來**:`cProfile` 的開銷與呼叫次數成正比,而這個工作負載有 **6,080 萬次**函式呼叫,所以百萬次級的 SQLAlchemy 機械被嚴重灌水,而 `fetchall`(201 次大呼叫)幾乎不受影響。隨機取樣外推出 44 分鐘、實測 28 分鐘——**這份也高估,方向與有偏那份相反**。唯一可信的外推是 `fetchall` 本身:58.66s × 19.91 ≈ **1,168s**,對上實際的 1,702s ≈ **69%**。
- 🔍 **所以「下一個槓桿是 `fetchall`」碰巧是對的,但產生它的 profile 是有偏的**。對的結論配錯的推理不比錯的結論安全多少——下次換一個瓶頸就不會這麼幸運。真正該記住的是:**外推之前先問取樣怎麼來的**,以及 profiler 對「很多小呼叫」與「少數大呼叫」的失真方向相反。
- 📌 **17:40 那輪第一次有完整儀表**(`daily-branches.sh` 的 `run_step`,09-22 上線):`import-branch-trades` **2,944s**(最大的一步)、`compute-branch-stats` 1,702s、`export-json` 1,157s、`deploy` 130s、其餘合計 &lt; 120s;整輪 17:40:01 → 19:20:54,**100 分鐘**。先前這一輪連 start/done 標記都沒有,耗時要從交錯的 log 反推。

## 2026-09-23 FontScaleToggle 掛了整套機制、只漏了畫面上那顆按鈕

- 🚨 **`docs/36` 記成 `[x]` 已完成的功能,其實從未出現在任何一頁**(`a5ae269`)。`UserPrefsProvider` 已掛、`globals.css` 三段 `html[data-font-scale]` 規則都在、`layout` 的防閃爍腳本早就在讀 `localStorage` 的 `font_scale` 並在首繪套用 `body.zoom`——**偏好設定會被記住、會被套用,就是沒有任何地方讓使用者去設它**。缺的只有一行:把 `FontScaleToggle.tsx` 匯出的元件放進某一頁。
- 🔍 **與 `a0c7b06` 是同一種形狀,但這次連測試壓力都沒有**。`a0c7b06` 是資料結構上不可能送達頁面;這次是元件本身從未被放上頁面。兩者都由「讀 JSON / 讀原始碼」的靜態檢查抓到,不是靠跑起來看畫面——本專案沒有 React test runner,這類缺口本來就照不到既有測試。
- [x] **修法連機制其餘三段一起釘住**:掛載測試檔新增第二份、刻意獨立的清單釘住按鈕本身,另加一條測試釘住其餘三段(provider、CSS 規則、防閃爍腳本)維持完整——因為上一次只有按鈕不見時症狀是「完全看不出來」,若之後改壞的是機制的另一段,症狀會變成「按鈕按了沒反應」,比「按鈕不見了」更難查。
- 🔍 **附帶的普查結果,記下來但本輪不做**:本專案沒有任何 e2e 測試;44 個元件裡有 20 個只有單一呼叫點——整站掛載清單是下一張票,刻意不讓它偷偷長在期貨測試檔裡面。

## 2026-09-23 兩個守門缺口:沒人驗證元件真的掛上頁面,比率閘門是逐路徑寫死的

- 🚨 **缺口一:沒有任何測試驗證元件真的被掛上去**(`55a0442`)。刪掉五個期貨 JSX 呼叫點的任何一個,881 個 python 測試與 47 個 node 測試全部照樣綠——它們測的是純函式與 payload,沒有一個碰過「這個元件出現在哪一頁」。`a0c7b06` 修的是同一種失敗的另一個成因(整個期貨切片結構上不可能顯示,測試卻全綠)。新增 `tests/test_futures_components_are_mounted.py`,解析 `.tsx` 原始碼、把註解與字串換成等長空白後驗每個元件「拿得到」且「畫出來」,並驗呼叫點不是只被另一個期貨元件畫出來。順手釘住 `FuturesDailyBlock` 與 `FuturesAnomalyBlock` 不得合併(一個是每天 320 個契約的日常事實,一個是個位數的例外旗標,`docs/38` §7.7)。
- 🚨 **缺口二:比率閘門逐路徑寫死,新鍵預設沒人守**。`futures_open_interest_direction` 上線那天,三把既有閘門全部照不到它,當時手動補了第四把——那只是把洞往後推一個鍵。改成**由命名規則發現表面**:走完整個 export 產物,凡是名字帶 `futures` 的鍵,連同底下整棵樹一起檢查,之後新增 `futures_*` 鍵不必有人動閘門。詞彙表合併成 RATIO_ISH + RANK_ISH(在整個表面這個尺度上「不自己做除法」與「不自己排名次」是同一條規則的兩半)。
- [x] **驗證通用閘門真的走到了,不只是看起來像覆蓋率**:釘住它走到五個入口、40 個以上的鍵,直接把違規鍵種進 payload 的三個深度證明它會紅。實測:在 export 端加一個 `futures_turnover_concentration.top5_share`,其他 103 個測試全綠,只有通用閘門紅。
- [x] pytest 888 passed / 3834 subtests(baseline 881;−3 舊閘門、+6 掛載、+4 通用閘門);`node --test futures.test.ts` 47 passed;`npm run build` 成功;`tsc --noEmit` 仍是既有的兩個 TS5097。

## 2026-09-23 期貨補到每個契約每一天,未平倉只讀一般時段,新增市場層級方向計數

- 📌 **~320 個契約每天都有量、未平倉、時段拆分,但只有掛旗標的那四個能在畫面上看到**(`2380a1b`)。`_futures_by_stock` 早就把這些數字寫進 `futures.contracts[].daily`,`web/` 從沒讀過。新增個股頁每契約每日事實區塊(成交量含一般+盤後合計與拆分、未平倉、日變化),沒有 `daily` 的契約什麼都不畫(缺席 = 未上市／未公布／匯入失敗,§7.7 禁止標「正常」)。`daily.oi_change` 對**每個**契約-日都顯示,不只掛旗標的四個——部位是增是減和成不成交量新高無關,綁在旗標上等於一年只講四天。**未平倉方向來自 battery 自身的 `oi_change()`**,不是在這裡重算一次,測試鎖住兩處數字相等,避免同一頁上兩個名字相近的數字互相漂移。`daily.volume`(兩時段合計)與 `anomaly.today`(僅一般時段,R3)現在真的會同框出現,故前者標「一般+盤後合計成交」而非「成交量」——標籤是唯一分隔兩者的東西(§7.6)。
- [x] **`daily.open_interest` 改為只讀一般時段,不再跨時段相加**(`bf28824`)。未平倉是存量不是流量,兩個時段的水位相加沒有意義——今天答案剛好正確,唯一理由是 TAIFEX 對盤後未平倉回傳 `'-'`;來源哪天開始供應盤後未平倉,水位會無聲翻倍而 `oi_change`(本就只讀一般時段)仍正確,同一張卡片兩個數字互相矛盾且沒有測試會紅。測試反事實地給盤後列一個未平倉值,鎖住水位不動。沒有 `daily` 的契約在個股頁畫一行「{code} 在 {daily_as_of} 沒有列」,只陳述缺席不猜原因。
- [x] **新鍵 `radar.json:futures_open_interest_direction`**:全部約 320 個契約裡,今天未平倉增加/減少/持平/判不出來各幾個,四個計數恆等於契約總數。**這一項不需要 §3 的 battery,理由要留著**:§1 的旗標做的是資訊性主張(可以被否證),這四個計數是描述性陳述,沒有虛無假設就沒有東西可以被否證。豁免只在它保持描述性時成立——沒有門檻、沒有旗標、沒有判語、沒有比率/百分比/淨額/總數、不排名契約。是平行的頂層鍵而不是塞進 `futures_volume_anomalies_meta`(母體不同、缺席條件也不同),`§7.11`「meta 只有一個日期加一個整數」的鎖維持不動。
- [x] pytest 881 passed / 3808 subtests;`node --test futures.test.ts` 47 passed;`npm run build` 成功;`tsc --noEmit` 仍為既有的兩個 TS5097。

## 2026-09-22 四輪日更原本一個失敗語音都沒有,補的過程自己又生出一個更嚴重的

- 🚨 **`install_fail_trap` 對最常見的失敗模式是裝飾性的**(`af327f1`)。`radar`、`compute-*`、`deploy_data` 都是 shell 函式,ERR trap 沒有 `set -E` 就不繼承進函式——`radar` 內部的 `( exit $rc )` 就地帶著原碼結束,lib.sh source 時裝好的 trap 完全抓不到。實測四種寫法:裸呼叫與現行的 `run_step X radar X || exit` 都不觸發、靜默中止;只有 `run_step` 沒接 `||` 才會觸發。也就是說 `daily-branches.sh` 的 compute-branch-stats、compute-scores、compute-performance、export-json、prune、deploy_data 全部靜默失敗,只有 cron log 記得——22:00 改純匯入後,17:40 是當天唯一的重算與上線,靜默失敗等於網站停在昨天直到 00:05 夜間作業(約 01:30)才補上,沒有任何人被告知。本專案最慘的一次事故正是四天靜默停擺。新增 `run_step_or_fail`:跑 `run_step`,失敗就在 `else` 取碼、發一則指名步驟與離開碼的 high 通知,再 `exit $rc`(用 `exit` 而非把非零丟回頂層,避免同一次失敗送兩則)。分點匯入與 `safe-branch-stats.sh` 刻意不改,它們的分級通知已經對。
- 🚨 **同一個洞在其餘四輪(14:10/15:00/16:10/21:20)一個字都沒補,`14:10`(最大的一輪)與 `15:00` 連失敗通知都沒有**(`9fed499`)。`run_step_or_fail` 原本的收尾句寫死引用 daily-branches 自己的後續契約(「本輪無完成標記,00:05 夜間作業會重算」)——對其餘四輪是假的:沒有一輪寫完成標記,夜間作業也只重算分點統計,不會重跑報價/法人/融資的匯入。改成每輪呼叫 `set_round_consequence` 自行宣告後果,`run_step_or_fail` 在後果未設定時**拒絕執行**任何步驟——忘記設定的腳本會在第一步就 exit 78 並發 high 通知,而不是靜默繼承別輪的說法。各輪的後果句照 crontab 與腳本本身據實寫:14:10→15:00 會重跑全部六步、15:00→16:10 會重抓 TPEx 報價並重新發布、16:10→17:40 會重新匯入法人並跑全套、21:20→沒有人接手(融資是唯一排程匯入者,只能人工補)。
- 🚨 **補洞過程自己引入一個更嚴重的 bug,被下一個任務接住**:`if` 測試表達式會讓 `set -e` 對整次呼叫失效(**包括被呼叫的函式內部**),而 `deploy_data` 結尾是 `cd "$REPO"`。用 `run_step` 包住 `deploy_data` 後,一次失敗的 `npx wrangler deploy` 會回傳 0——完成標記照寫、`notify_ok` 照發、00:05 的夜間備援因此被跳過:一次失敗的發布看起來像成功,而且順手關掉了備援。實測:裸呼叫 `deploy_data` rc=9,包了 `run_step` 之後 rc=0。已修成讓它像 `radar()` 一樣自己截碼,兩者現在都會回 9。**自 `6907eff` 起已在夜間排程上活了 7 天**;沒有真正的部署失敗發生在這段期間,所以沒有東西真的被蓋掉,但這是設計上「一個測試檔案的修法本身可能引入新缺陷」的活例。
- [x] **`run_step` 從 `safe-branch-stats.sh` 搬進 `lib.sh`,17:40 這一輪第一次有任何時間量測**(`25c6f91`),連 start/done 標記都是新的——先前這一輪的耗時只能從交錯的 log 反推。兩輪(17:40、00:05/22:00 那條鏈)現在吐出同格式的 `step <label> done rc= elapsed=` 行,可以直接 grep 比對同一步驟。離開碼逐位元保留(0/75/76 分級不受影響,已用 0/75/76/2 的假 harness 驗證),ERR trap 不會因為換了計時包裝而多噴一次。

## 2026-09-22 新鮮度徽章對自己說謊:把 futures 加進 `FRESH_LABEL` 卻忘了排除它

- 🚨 **驗證者追一條沒人指過的路徑,抓到修一半的修復**(`f38fe50`)。`futures` 加進 `FRESH_LABEL` 卻沒同時排進徽章的過濾清單,於是它落進共用樣板「個股期貨今日尚未公布,暫用 {date}」——對這個結構上永遠不可能有今天資料的資料集講一句不加限定的「今日」。`json_export.py` 的摘要句子當時已經補了期貨排除,這一處沒有,兩者本應同步。
- [x] **期貨現在有自己的句子,講出它真正站在哪一天,以及「過期」對它的意義**——落後前一個交易日,不是「今天還沒到」。摘要句尾端「依交易所公布時間分批自動更新」在期貨是唯一過期資料集時一併拿掉:這條管線兌現不了這個承諾,期貨的端點只給最新一天,漏掉的一天不會自己補回來,得跑 `backfill-futures`;同一句假承諾也從 `summary_text` 移除。兩個判斷都搬進 `lib/freshness.ts` 的純函式並補測試——這個缺陷是**休眠的**:只有期貨落後超過正常的一天延遲才會被踩到,沒有測試就會一直休眠下去。

## 2026-09-22 方向 battery 在**補好報價缺口之後**重跑:裁決不變,面板留下

- 📌 **為什麼要重跑,而這不是「跑到滿意為止」**。2026-09-08 那次裁決(backward obs/exp = 2.396 對事先寫死的撤回門檻 1.5)是在**帶著 25 個交易日報價缺口**的資料上算的——缺口 2026-07-16..08-19,TPEx 當時每天只有約 487 列(修復後 1,015),而它**落在 battery 的 490 日窗口內**。裁決本身沒有錯,但它讀的資料九天後被補齊了(修復窗 2026-09-19,+15,626 列)。**處置在跑之前就寫死在腳本註解裡**:沿用同一條 1.5 門檻,不新增不放寬,掉到 1.5 以下就照原承諾撤下面板。
- [x] **實驗設計:與 2026-09-08 那次完全同參數**——同 `as_of=2026-09-08`、同 490 日窗口、同預設 seed。**唯一的差別就是修好的資料**。換成今天的 as_of 會把窗口整個往後推,那是另一個實驗,答不了這一題。
- [x] **Backward(已上線的面板)三項全 KEEP**:`backward_collapse_toward_placebo` obs/exp **2.33442**(門檻 1.5);`backward_vs_flow_matched_placebo` exceeds-both **0.5625**(n=160)對流量配對安慰劑 **0.2833**(n=120);`backward_lag_test` 2.334 → **2.225**(門檻 0.5× = 1.167,保留率 95%,前次為 90%)。兩條「再看一次」提示線(survivors 100、obs/exp 2.0)皆 `[REVIEW NOT TRIGGERED]`,survivors = 160。
- [x] **Forward 仍 WITHDRAW,而且差值恰好是 0**:`forward_vs_flow_matched_placebo` exceeds-both **0.3158**(n=19)對安慰劑 **0.3158**(n=19),**difference 0.0000**,2σ = 0.3016。往前看那側與安慰劑完全無法區分。它本來就沒上線,這次是在修好的資料上再次確認不得加入。n=19 本身也是資訊:往前看那側幾乎沒有存活者。
- 🔍 **結論:2.396 → 2.334,差 2.6%,離門檻 1.5 很遠——當初的通過不是報價缺口造成的假象。** 原始 JSON 存於 `docs/evidence/direction-battery-rerun-20260922.json`。
- ⚠️ **第一次重跑失敗,原因值得記**:把 `data` 目錄掛成 docker `:ro` 會直接開不了庫——WAL 模式下即使 `mode=ro` 的讀者也要在目錄裡建 `-shm`。先前期貨 battery 用同樣掛法會成功,只是因為當下 WAL 剛好被 checkpoint 成 0。唯讀是由**程式碼**(`get_read_only_sqlite_engine`)保證的,不該靠掛載旗標;先前驗證者已用位元組比對證明它不寫任何一列。

## 2026-09-21 §4 步驟 4 執行完畢:期貨成交量異常**通過**事前登記的檢定,裁決 SHIP(第 1 次執行)

- 📌 **依 §3.4 的紀錄義務,逐項寫下**。事前登記 commit **`c70f1c2`**(修訂 `aff30b5`);回補完成時 `futures_daily` = **250 個交易日 / 485,686 列**;`as_of` = **2026-09-17**(期貨與現貨都已匯入的最後一天,期貨到 09-17、現貨到 09-18,取較早者);**第 1 次執行**;320 個契約乘數**全部已知**。原始 JSON 存於 `docs/evidence/futures-volume-battery-20260921.json`——先前有驗證者指出效能數字在 repo 裡查不到出處,這次把證據一起進版控。
- [x] **檢定 A(區辨性)PASS**:`|F_only| = 680 ≥ 30`。`|F| = 1052`、`|F_only| + |F_(S=1)| = 1052`、`f_with_spot_new_high = 372`、不成熟 5、**`spot_flag_unknown = 0`**。
- [x] **檢定 B(資訊性)PASS,10 個 seed 全過**。`n = 680`、`h_F = 141`(20.7%);安慰劑 `h_P` 在 **43~67** 之間(約 8.2%),`σ_P` 6.35~7.77。最差的一個 seed 是 `141 − 67 = 74`,仍有 **9.5σ** 餘裕,遠超過 2σ 門檻。安慰劑池 25,055 天 / 215 檔,**抽不滿的股票 0 檔**。
- [x] **裁決:`a_and_b_passed` → SHIP**。依 §4 步驟 5,此後才可以實作 export 切片。
- 🔍 **否決計數**:`R1_window_gap = 19,097`、`R2a_zero_median = 1,052`、`R2b_immaterial = 4`、`R2b_multiplier_unknown = 0`、`R2b_spot_history_gap = 0`。涵蓋面:評估 61,326 個契約-日,其中結算窗內 16,640、無一般時段列 2,034、無現貨日曆 0。
- ⚠️ **`sets.f` 與 `R2a_zero_median` 剛好都是 1,052,查過了,是巧合不是重複計數**。`_refused()` 回傳 `{"refusal": code, "flag": False, ...}`,被否決的日子 `flag` 恆為 False、永遠進不了 F,兩者結構上互斥。記下來是因為這種巧合值得查一次,而不是看到就放過。
- 📌 **`docs/38` 新增 §7「步驟 5 的補白」,它是 post-data 的,地位與 §1–§6 不同**。§1–§5 是事前登記(`c70f1c2`),§6 是回補資料仍不可見時寫的增補(`aff30b5`)——兩者都凍結;**§7 寫在 battery 跑完、裁決 SHIP 之後,作者已經看過資料**,所以它**不享有事前登記的效力,也不得被引用來放寬任何一條規則**。它只記錄「同一套結果怎麼呈現」:`oi_change` 缺值時句子怎麼收尾、現貨旗標未知時風險句怎麼收尾、兩段文字掛在哪裡用什麼代碼、`anomaly` 是四或五個鍵、`radar.json` 新增的市場層級名單 `futures_volume_anomalies`(三態 + `today − window_max` 的順序)、`daily.volume` 與 `anomaly.today` 是兩個數字、缺 `anomaly` 的三種意思都不叫「正常」、21:20 匯入造成的可見性窗口、藥丸顏色走中性、以及「絕不依股票去重 / 不要在前端寫死 60」。**它一個門檻、一個數字都沒有動**(60 / 3 天 / 1% / 2σ / 30 / 5 天 / seed 0..9 全部原封不動);把 §7 整節刪掉重跑 battery,`|F|`、`|F_only|`、`h_F`、`h_P` 與裁決一個字都不會變。§3.5 的禁止清單完好。
- 🔍 **兩個事前爭點,現在各自有答案**:修訂 4 保護的「成熟但 `S` 未知」族群實測為 **0**——實作者說「幾乎不可達」是對的,驗證者說「不能事先證明可忽略」也是對的,現在它是**被量出來的 0**,不是被假設的 0。R3 的盤後問題由定義解決,歷史裡確有盤後列(13,497/485,686)但統計量從不讀取。
- [x] **§4 步驟 5 的 export 切片落地(`7d8e198`),不擁有規則,只重用 battery 的**:`_oi_change` → `oi_change`、loader 轉 public,新增 `evaluate_contract_day_in_calendar` 把 window builder、純規則與 `oi_change` 併成單一進入點,`_assemble` 與匯出共用同一個呼叫點,不是兩份邏輯各自維護。`anomaly_facts` 把結果轉成 §1 五個整數鍵或 `None`——`None` 同時代表「被否決」與「規則跑過、沒有旗標」,和 `futures` 鍵本身的三態一致,不能印 0 或掰一句 not-published 去填缺值。Mutation 檢測 21 處存活 19,兩個倖存者當時判為等價(60 已被 §3.5 凍住、`FrozenConstantTests` 鎖住;匯出落後匯入日時的提前 return,與逐契約「今日無列」檢查等價)。**第二個判斷是錯的,而那個倖存者就是下面那個 bug**:它在 fixture 裡等價(fixture 讓期貨與現貨同一天),在 production 裡不等價(期貨常態落後一天,那個 return 於是每天都走)。教訓不是「mutation 檢測沒用」,是**倖存者的等價性只在 fixture 的形狀之內成立**——fixture 的形狀不對,等價判斷就跟著不對。
- [x] **首頁分頁與個股頁上線(`0b126e6`),判斷邏輯鎖在 `lib/futures.ts` 的純函式、`node --test` 守住**——因為 §7 每一條都是「不可以顯示什麼」,讀 JSX 驗不出「不顯示」。市場層級鍵三態不塌成兩態(缺鍵不顯示檔數,印 0 會把「沒算過」講成「今天沒有異常」);`oi_change` 缺鍵整列不顯示、不補 0、不補破折號;不算比率、不排名次;單位是契約,任一層都不依股票去重(1565 的 `MYF`/`OMF`);`window_days` 從區塊讀,前端不寫死 60。**上線時的已知缺口**:空陣列那一態當時沒有 `window_days` 可讀,句子只能說「沒有契約創新高」,講不出「幾天的比較窗」。
- [x] **上一條缺口當天修掉(`8b57ac1`)**:§7.10 禁止前端寫死 60、規定讀 `anomaly.window_days`,但「算過、沒有旗標」那一態的陣列是空的、整個 `anomaly` 區塊不存在,句子沒地方讀數字——UI 因此上線了較弱的文案,**缺的是 payload 不是文字**。新增平行的頂層鍵 `futures_volume_anomalies_meta: {window_days}`,取平行、不取「把陣列包進物件」:包物件會把「沒算過」與「算過但空」逼成同一物件裡靠一個欄位分辨,正是 §7.5 要防的塌陷。數字來自 `futures_volume_battery.WINDOW_DAYS` 本身,不是第二個字面 60,記為 `docs/38` §7.11。
- [x] **期貨異常切片從未渲染過,今天修好(`fix(futures):`)**:錨點錯了——export 拿**現貨日**去問期貨,而期貨資料日**常態就是現貨日 − 1 個交易日**(餵源只給最新一份完整日報,21:20 那一輪最新的完整日報是前一天的;§7.8 的舊記錄把這件事寫反了,一併更正)。於是 `contracts[].daily` 的 `WHERE date = :d` 永遠撈不到列,`futures_volume_anomalies` 的 `futures_days[-1] != as_of` 永遠回「沒有算過」,市場層級的鍵**一次都沒有出現過**——唯一會讓它通過的日子是現貨匯入失敗那天。production 佐證:`futures_daily` max 2026-09-18 vs `daily_prices` max 2026-09-21。修法是把 `f_date` 這**一個**變數串進整個切片(`freshness.futures.date`、`daily`、新的 `futures.daily_as_of`、旗標、`_meta.as_of`),日期在兩個 payload 裡**明講**不推導,UI 每一句有主張的話都帶期貨行情日、不用沒有限定的「今日」,記為 `docs/38` §7.12。**根因在測試**:fixture 讓期貨與現貨最後一天相同,production 的形狀從來沒有被建模過,所以一份全綠的測試放行了一個結構上不可能渲染的功能——「現貨比期貨多一個交易日」從此是**預設**形狀,同一天反而要顯式關掉。順手兩件:`freshness.futures.stale` 不再恆為真(改成「連前一個交易日都沒跟上」),期貨移出「稍後自動補齊」那句話(這條管線補不了,只能人跑 `backfill-futures`),`FRESH_LABEL` 補上 `futures`。
- [x] **R4 在日曆邊緣的正確性修正,記為 §7.13**:`settlement_date()` 定位不到結算日時回 `None`,而 `settlement_exclusion_days()` 把它變成空清單——與「這個月沒有排除日」不可分辨。battery 帶著事後日曆跑,這個塌陷永遠不會發生;export 永遠坐在日曆尾巴上,它會發生,方向是**放行**結算日前 1–3 個市場日,也就是**上線的規則與被檢定過的規則在邊緣不同**。裁決:**不可判定 ⇒ 不主張**(回 `None`,不是 `{}`)。「離窗口夠遠」用已知市場日數,已知日曆之後把平日當市場日——這個外推是本節唯一的取捨,且錯誤方向與修正前完全相同(所以任何一天都不會更差);若改採「定位不到就一律不主張」,每個月從 1 號到第三個星期三的每一天都會停止主張,那是關掉不是保守。市場日日曆改讀 `data_date`,白拿現貨已經有的那一天先見之明。
- 🔍 **本輪查出三處「相沿記錄與實際不符」,記下來因為這是查證的價值所在**:①先前記錄說 day-t 旗標從約 01:30 的夜間發布才看得到——錯,`daily-margin.sh` 在 21:20 同一輪就依序跑 `import-futures`、`export-json`、`deploy_data`,可見時間其實是約 21:2x,已在 §7.8 更正;照抄原記錄會讓未來的驗證者查錯發布時間點。②記錄引用 `stock.reasons` 作為 `{code, text}` 形狀的先例——查證後 `stock.reasons` 其實是 `string[]`(舊式純文字清單),只有 `stock.raw_reasons` 與 `technical.reasons` 才是 `ReasonItem[]`。③(**這一項發生在 2026-09-18,不是今天**;commit `cff14c0` / `2b5038a`,寫在這裡是因為它與①②是同一種錯誤形狀,但日期不該被這個編排帶偏)有提案要把 `warrant-backfill.sh` 改成 `if ...; then` 的 exit code 判斷寫法,**已駁回**:該腳本的擷取是管線(`radar ... | tee`)上的 `${PIPESTATUS[0]}`,`if` 形式改抓的是 `tee` 自己的狀態,兩者只在 `tee` 本身失敗(該腳本的典型失敗模式是磁碟耗盡)時才不同——那時乾淨可續跑的停止(75)會被讀成真失敗(1),既誤報又打斷 driver 迴圈;而且該腳本早就自行解除了自己的 ERR trap,這個 bug 從未真的發生過。系統性排查後在 `mid-backfill-publish.sh` 找到真正的實例——它解除 trap、經 `install_fail_trap` 重新掛上、再用 `set +e` 擷取——已修正。
- [x] **修復窗口於 2026-09-19 執行,刻意不匯出不部署**:報價缺口 28 個低量日 → 0(+15,626 列)、point-in-time 帳本缺 4 天 → 0、`forward_returns` 747 → 10、融資 2026-09-02 1,293 → 2,214 列。「發布是另一個決定」——本輪未動正式匯出與部署。
- [x] **`forward_returns` 的 binary search 已在正式機重跑同一份 profile 驗證**:20 檔 95.46 秒 → 13.83 秒,線性掃描的 generator expression 消失,SQL/Python 比從 7/93 反轉為 76/24;外推到 `compute-branch-stats` 約從 4,627 秒降到約 11 分鐘。下一個槓桿是 `fetchall`,目前佔剩餘時間的 65%。
- ⚠️ **仍然打開,如實記下**:①前端有兩處接線缺口沒有測試覆蓋(本專案沒有 React test runner)——空狀態句子在 `page.tsx` 直接寫死字串、沒呼叫 `anomalyEmptyStateText`,以及 `meta` prop 在傳遞中被漏掉;兩者的劣化方向都誠實(退回沒有窗口數字的句子,不會退成假的 60)。②市場層級名單的 `code` tiebreak 沒辦法透過 `export_json` 證偽(`load_contracts` 早已 `ORDER BY contract_code`),只能靠直接呼叫 `anomaly_index` 帶反序輸入來測。③一張卡若只掛旗標藥丸與風險藥丸兩者,風險藥丸視覺上比旗標藥丸搶眼——這是 §7.9 指定的行為(對描述性旗標保持中性),記下不是要改。④17:40 那輪的覆蓋率地板仍在其 10 個交易日的事前登記觀察期內(約到 2026-10-01)。⑤§7.12 的 UI 文案有四處只活在 JSX 裡、沒有測試(本專案沒有 React test runner):`FuturesAnomalyList` 的「沒有算過」那句與名單標題、個股頁 `FuturesAnomalyBlock` 的標題日期、`page.tsx` 的 `FRESH_LABEL`。能抽成純函式的都抽了(`anomalyEmptyStateText`、`anomalyLagText`、`anomalyFacts` 的標籤,全部被 `node --test` 鎖住並通過 mutation 檢測),剩下這四處是排版本身;劣化方向誠實(少一個日期或印出原始鍵,不會編一個日期出來)。

## 2026-09-18 夜間 77 分鐘的 87% 是一行線性掃描;改 binary search,逐值對照舊實作

- 🔍 **量測方法與前提,先寫清楚**(驗證者指出原本的數字在 repo 裡查不到出處,這是對的批評)。`cProfile` 跑 `compute_all`,**N=20 檔**,DB 以 `mode=ro` 掛載、`init_db` 停用,**沒有寫入任何資料表**;機器同時在跑權證爬蟲(網路受限,約每秒一次請求),所以**絕對秒數是上界**。20 檔共 **95.46 秒**,`tottime` 前三名:
  - `performance.py:33 <genexpr>` **83.32s(87.3%)**,ncalls **140,087**
  - `sqlite3.Cursor.fetchall` **6.98s(7.3%)**,ncalls **41**
  - `compute_all` 自身 **1.40s**,而 `forward_returns` 被呼叫 **70,114** 次
  SQL/Python 比約 **7 / 93**。原始 pstats 檔沒有進版控,上面這幾個數字就是全部的紀錄。
- 🚨 **競爭這個前提不影響結論,因為機制是從程式碼讀出來的,不是從計時推論的**:`forward_returns` 拿到的 `candles` 是整檔股票的完整價格歷史、**已排序**(docstring 本來就寫著),而它用 `next(i for i, c in enumerate(candles) if c["date"] > signal_date ...)` **從 index 0 線性掃描**。約 2,500 個交易日的歷史平均掃一半 ≈ 1,250 次 dict 存取,正好對上實測的 0.6 毫秒/次。已排序的序列找「第一個大於 X」是二分搜尋。
- [x] **改成 `bisect_right(candles, signal_date, key=...)` 之後再向前步進到第一個可用開盤價**。向前步進那個迴圈**是承重的,不是防禦性寫法**:`compute_branch_stats` 與 `branch_ranking_v2_shadow` 兩個呼叫端用 `(r[1] or 0) * (r[3] or 1.0)` 組開盤價、**沒有** `open IS NOT NULL` 過濾,所以 0 開盤價真的會出現;少了那個迴圈會靜默改掉進場價。另注意負開盤價對 `c.get("open")` 為真,必須靠 `> 0` 擋掉。
- [x] **驗收標準是「數字一模一樣」,不是「測試通過」**。舊實作逐字保留在測試裡當 oracle,比對**整個回傳 dict**,3,000 筆固定種子隨機案例;驗證者另外用**自己寫的** oracle 跑 3 萬筆獨立隨機比對,**0 筆不符**。三個呼叫端全部來自 SQL `ORDER BY date`,可證明排序。複雜度用**計數**釘住(4,000 筆歷史讀 20 次欄位;線性掃描要 3,900 次),並經證明不是套套邏輯——把 binary search 換成行為相同的線性掃描,parity 全過而複雜度測試失敗。
- 📌 **效能與誠實的預期**:單檔函式實測 60.3 µs → 5.1 µs(**11.8×**),驗證者在自己的合成歷史上量到 **35–39×**。但 `compute-branch-stats` **不會**因此趨近於零:扣掉那一行之後,`fetchall` 與每次呼叫的 dict 組裝仍在,預期 **77 分 → 約 12–18 分**。**下一個槓桿不是繼續微調這個函式**,而是 `compute_all` 用**同一個連線包住整個逐檔迴圈**、以及迴圈內每檔兩次 `fetchall`。

## 2026-09-18 250 交易日期貨回補完成、契約乘數落地(§4 步驟 2 與步驟 3 收尾;步驟 4 仍未執行)

- [x] **回補完成 2026-09-17T23:36**。依 `docs/38` §4 步驟 2,這裡**只記日期數與列數**:`futures_daily` = **250 個交易日、485,686 列、320 個契約**,`range=2025-09-08..2026-09-17`,`chunks=13 still_missing=0 errors=0`,磁碟 6.2G → 6.1G。這兩個數不是 §1–§3 任何門檻的輸入。
- 🔍 **R3 當初說「回補才會揭露」的那件事,揭露了:盤後在歷史裡確實有列**——一般 **472,189** 列、盤後 **13,497** 列(2.8%)。R3 在看到這個數字之前就已經定死「統計量永遠只用一般時段」,所以它**不能**、也沒有反過來影響任何門檻;記在這裡是因為 §2 R3 自己寫明了這是回補會揭露的事實。
- [x] **`contract_multiplier` 有寫入端了(`f3af5d5`)**,值取自台期交標的清單頁第 11 欄「標準型證券股數/受益權單位」,**逐列讀,不從契約類型反推**。
- 🚨 **一個我原本的假設被實測推翻,記下來免得日後有人再假設一次**:上一節寫「標準型 2,000 / 小型 100 / ETF 另計」,而 ETF 並**不是**一律 10,000。320 個契約的實際分布是 **2,000 × 249、100 × 47、10,000 × 21、1,000 × 3**——那 3 個是**小型 ETF 期貨(1,000 受益權單位)**,是活的商品不是選單殘留。若照「ETF ⇒ 10,000」推,會錯 3 次;照「股票 ⇒ 2,000」推,會錯 47 次。**320/320 全部讀得到,零空白、零非數字。** 1565 的 `MYF`=2000、`OMF`=100 是同一檔股票兩個乘數的實證。
- [x] **未知一律留 NULL,而且不覆蓋既有值**(`coalesce(excluded, 既有)`)。台期交不會公布「乘數未知」,所以 `None` 永遠是**我們這邊讀失敗**;把它寫進去會抹掉一個已知值,而 R2b 對未知是否決,那個契約就會**無聲且永久地**停止產生旗標。真的改乘數一定帶著數字,非 NULL 照常覆蓋,R5 的偵測空間不受影響。
- ⚠️ **§4 步驟 4 仍未執行,而且現在還不能執行**:正式機的 `futures_contracts.contract_multiplier` 目前仍是 **320 列全 NULL**——寫入端要等下一輪 `import-futures`(21:20)跑過才會填。在那之前跑 battery,R2b 會因「乘數未知」否決全部 320 個契約而得到 `|F| = 0`,**那不是結果,是還沒準備好**。裁決、`|F|`、`|F_only|`、每個 seed 的 `h_F / h_P / σ_P` 一概還不存在。

## 2026-09-17 覆蓋率地板第一次真的開火:健康的來源在 17:40 只填到 46%,改由當天第二輪備援上線(閘門不動,觀察 10 個交易日)

- 🚨 **閘門正確地擋下了一輪,但擋的不是它當初要抓的東西**。`branch_coverage` 五列實測(coverage/expected):09-15 22:49 = 1956/1956、09-16 18:27 = **1688/1957 = 0.8625**(17:40 那輪)、09-16 22:44 = 1957/1957、09-17 18:30 = **902/1956 = 0.4611**(17:40 那輪,**掉出 0.5 地板**)、09-17 22:45 = 1956/1956。09-17 17:40 那輪的匯入計數是 **1412 ok、1054 empty、0 failed**,empty 全是 NoDataError ——**來源是健康的,只是 18:30 這個時間點來源還沒把那些股票公布出來**。地板在設計時被記成「實際上只有來源死掉/佔位頁那種落在 0 附近的故障構得到」,這一天把那句話否證了:在 17:40,地板也會被**來源太晚**觸發。
- 🔍 **當天的代價,如實記下**:17:40 那輪 exit 1、不重算、不上線、不寫完成標記——這部分全部正確。但 22:00 已經改成純匯入,於是**整個晚上沒有任何一輪上線**,00:05 的夜間作業成了當天第一個發布者(約 01:30)。舊排程下 22:00 那輪約 00:25 就會上線。
- [x] **修法:閘門一個字都不改(0.5 地板、0.8 告警,兩輪都一樣),改的是 17:40 失敗之後會發生什麼——當天沒有完成標記時,22:00 那輪接手完整鏈**。**理由是承重的**:withhold 邏輯只能**阻止**上線,永遠無法**撤回**已經上線的東西。若改成把 17:40 的地板調鬆、讓它帶著半殘的資料上線,之後 22:00 即使看出問題,能做的也只有自己不上線——17:40 已經放上網站的東西還在原地。那等於把最鬆的閘門裝在唯一**分不出「還在填」與「死了一半」**的那一輪上。所以答案是備援,不是更低的地板。判斷條件用 `-s`(非空)不是 `-f`:標記內容是完成時刻,空檔案代表寫的過程出了事,不算「今天上線過」。接手的分岔放在離開碼 `case` **之後**,第二輪自己不合格時仍在 case 就 exit,不會帶著壞資料接手。
- ⚠️ **`BRANCH_ROUND_MODE=import` 這個名字從此比行為窄,刻意不改名**。它現在的意思是「當天的第二輪:匯入,然後只在今天沒人上線過時才上線」。改名要再動一次正式機 crontab,那是使用者的決定、要另外核准,不在本輪範圍。名字留著,真正的意思寫死在 `daily-branches.sh` 開頭的標題註解裡,讀的人不必從程式碼裡自己發現。
- [x] **不合格通知按輪次分級,但只有一份 `*)` 分支**(措辭與優先權在進 case 之前選進變數)。第一輪掉到地板以下 → `notify_warn`,說明第二輪會接手:有了備援,這件事**不需要任何人動手**,報成「失敗」正是 75/76 那段註解一直在對抗的「把一個正常結果講成故障」。第二輪掉到地板以下 → `high`:今天沒有任何一輪上線,夜間作業只重算、不上線。
- [x] **`branch_coverage` 那一列補上 `done=` / `empty=`**(沿用既有的自由文字欄,無 schema 變更)。這次的診斷能做出來,靠的是 cron log 裡那句 `1412 ok, 1054 empty`——而 `disk-cleanup.sh` 會修剪那份 log。低覆蓋率的兩個成因(來源死掉 vs 來源太晚)只看 coverage/expected 分不出來,放進 DB 之後,同樣的診斷日後光靠資料庫就重建得出來。
- 📌 **量測規則寫在量測之前,這是刻意的順序**。閘門維持原樣**觀察 10 個交易日**(約到 **2026-10-01**),讀的是兩輪各自的 `branch_coverage` 列。**判準事先定死**:10 天裡第一輪掉到 0.5 以下的天數若是 **0 或 1 天**,地板照舊——備援吸收得掉;若 **3 天以上**、而第二輪始終正常,那就表示 17:40 的這道地板量到的是**來源的遲到**而不是**來源的健康**,屆時它變成一個產品問題(「覆蓋率低到多少時,20:30 上線的價值低於等到 00:25」),要帶著數字去決定,不是在這裡臨時調。另外記下一個期望值:**第二輪應維持 ≥ 0.95**。**之所以先寫規則再看資料,是為了讓日後真的要改的時候,那是在套用一條事先講好的規則,而不是在把一個正在響的警報關掉。**

## 2026-09-17 事前登記 §6 修訂：安慰劑對照組修回「平常的日子」，另五條確認與澄清（仍在回補資料可見之前）

- **這份修訂與 v1 同等地位,理由是時間順序,不是內容**。`docs/38` §0 的凍結規則觸發在**資料可見性**上,**不是觸發在某一個 commit hash 上**:250 交易日回補**仍然沒有執行**(`futures_daily` 依舊 1,969 列、只有 2026-09-15 一天),沒有人對回補資料下過任何查詢,本輪所有測試的期望值一樣來自手算與自己造的 fixture。**這一節 commit 之後,規則才是真的定死了**;`c70f1c2` 仍是 v1 的 hash,battery JSON 另外帶一個 `metadata.preregistration_amendment` 指認本節。
- 🚨 **修訂 1(唯一會影響裁決的一條):R2b 的實質性不等式是「候選日的篩子」,不是安慰劑資格的閘門**。舊寫法對**每一個**契約-日評估 `100 × (today − window_median) × multiplier ≥ spot_median_shares`,於是平靜日(`today ≤ window_median`,超出量非正)結構上不可能通過,而 §3.3 的合格日定義是「R1–R5 全過而 `flag = 0`」——**安慰劑池因此只剩下「高但沒破紀錄」的日子**。那是把假設從「創高 vs 平常」偷換成「創高 vs 接近創高」,**不是同一個假設的比較嚴格版**,「它比較嚴格」不構成辯護。它還會自我毀滅:薄契約(窗口中位數 3 口,對上 5,000,000 股的現貨日常量)幾乎沒有平靜日跨得過實質性,但它真正的新高輕鬆跨過——**池子恰恰在會舉旗的地方餓死**,只要一檔薄契約舉兩次旗而池子只剩一天,檢定 B 就變成「不可評估」,而這種餓死再多交易日也補不回來,§3.5 的重跑規則救不了。**改法**:不等式移到 `today > window_max` 之後;創高卻過不了實質性者以 `R2b_immaterial` 否決,依 §2 標頭**兩臂都不進**。R2b 的**前置條件**(乘數已知、`W` 上現貨窗口完整)留在原位,因為那描述的是脈絡讀不讀得出來,與當天量大不大無關。合成 fixture 上,單一契約的池子由 13 天左右回到 **69 天**。
- [x] **修訂 2:成熟 = 往後五天的 `S` 五個都算得出來**。§3.1 寫的是不可評分的日子「不進分母、**也不算未命中**」;舊實作只數「有沒有 5 天」,窗口裡 `S` 為 `None` 的日子照樣進分母,而 `forward_hit` 把 `None` 讀成沒命中——後半句被違反。任一個未知 → t 從旗標臂、安慰劑臂與安慰劑池三處一起排除。
- [x] **修訂 3:成熟性同樣管檢定 A**——**既有行為已正確,本輪只是確認並把理由記下來**:§3.1 是「樣本」這一節,管的是整個 §3;§3.3 寫 `n = |F_only|`,這寫法只有在 F_only 本來就只含成熟成員時才說得通。`f_all_including_immature` / `f_immature` 照舊輸出。
- [x] **修訂 4:「冗餘」不得經由 `S` 未知的日子成立**。冗餘的條件改記為 `|F_only| + |F_{S=1}| ≥ 30` 且 `|F_only| < 30`。**冗餘是一個結論**(會寫進本檔告訴使用者「這功能沒有獨立內容」),而那句話只關於 `S = 1` 的日子;靠「讀不到」的日子湊到 30,結局是**檢定力不足的無結果**,不是冗餘——前者依 §3.5 可重跑,後者是收案。
- [x] **修訂 5:三條既有行為確認,外加兩點報告義務**。旗標日自己仍排除在安慰劑池外(§0 點名的 1565 MYF/OMF 雙契約情形正是會咬到的地方);抽不滿仍判**不可評估**,並補明 (i) 不可評估 = 不上線,且對 §3.5 的重跑規則算**無結果**;(ii) JSON 逐檔列出 `(stock_id, k, pool_size)`——差額與 seed 無關,讀者必須分得出「結構性餓死」與「再等幾個月就補得滿」,一個總數辦不到。`S` 未知的旗標日仍不進 F_only(未知 ≠ 否)。
- [x] **修訂 6:三處澄清**。`spot_median_shares` 取**下中位數**(同 `window_median`,同一個「只留整數」的理由,方向是六十個裡的一個名次、偏易);**F 日之前的日子刻意不排除**——t−1 的安慰劑與 t 的 F 共用四個往後日,這個滲漏讓 `h_P` 跟著 `h_F` 走,方向是**更難過關**,事前登記成一個**不可用來補救**的已知滲漏,檢定 B 失敗後不得拿它重做;**`as_of` = 期貨與現貨都已匯入的最後一天**,成熟性數 `≤ as_of` 的現貨交易日(§3.1 字面是「在 as_of 之前」,此處把落差定死)。
- 📌 **契約乘數的來源與查閱日,寫下來給日後覆核用**:**TAIFEX「股票期貨契約規格」**——標準型 **2,000 股/口**、小型 **100 股/口**、ETF 期貨另計(`schema.py` 該欄註解逐字記著同一組事實),**查閱日 2026-09-17**。`futures_contracts.contract_multiplier` 目前**沒有任何寫入端,既有列一律 NULL = 未知**,填值是另一個決定。**覆核者要注意的是**:本欄只會存「現在的」乘數、沒有歷史(§2 R5 的已知盲點),所以要判斷評估期內某個契約的乘數有沒有變過,必須自己去查 **TAIFEX 的契約調整公告**(減資、分割、合併會觸發),涵蓋範圍是 battery 實際跑的那段評估期——不能靠這一欄回答。
- [x] pipeline pytest **685 → 696 passed、282 subtests**(全部新行為逐條變異驗證:破壞一處、確認有具名測試抓到、還原)。合成 fixture 的安慰劑池大小(69 / 59 / 93 / 92 天)全部是**手算**的,不是跑出來抄回去的。
- ⚠️ **§4 步驟 4 仍未執行**:回補未完成,本輪一樣**沒有**產生任何裁決,`|F|`、`|F_only|`、每個 seed 的 `h_F / h_P / σ_P` 都還不存在。

## 2026-09-17 期貨成交量異常事前登記 commit（`c70f1c2`）與三項前置實作落地，battery 尚未執行

- **事前登記的 commit hash 是 `c70f1c2`**(`docs/38_futures_volume_anomaly_preregistration.md`)。**它在任何人對 `futures_daily` 的 250 交易日回補資料下過第一個 SELECT 之前就 commit**——撰寫當下該表只有 2026-09-15 一天(1,969 列,已用於 export),回補仍在進行中。這份文件的效力來自這個時間順序,不來自內容:§1 的統計量、§2 的五條否決、§3 的兩條全案否決門檻(30、2σ、5 天、60 日、1%、seed 0..9),每一個數字都在看到資料之前定死。要改只能開 v2,而 v2 只能用 v2 commit 之後才出現的交易日重新取得資格(同 `docs/04` §2)。
- [x] **§4 步驟 3 的三項前置實作完成,而且同樣是在沒有看過回補資料的情況下寫的**。本次 commit 的作者沒有對 `futures_daily` 查詢過任何一列;所有測試的期望值都來自手算的月曆與**自己造的合成 fixture**(一天 100 口的底、每七天一個 200 口的中量日、五個嚴格遞增的尖峰),沒有一個數字取自真實的期貨量。「先看看典型成交量長什麼樣子」正是這份文件要擋的動作。
  - **`futures_contracts.contract_multiplier`(Integer, nullable)**:沿用 `db.py::_migrate_sqlite` 既有的 additive migration 形狀(`PRAGMA table_info` 檢查 → `ALTER TABLE ... ADD COLUMN`),既有列一律 NULL。**NULL = 未知,不是 2,000**:§2 R2b 明訂乘數未知即否決該契約該日,匯入端也刻意沒有寫入這一欄——填值是另一個決定。
  - **結算日曆 `compute/settlement_calendar.py`**:純月曆的「第三個星期三」與需要日曆的「順延／前 3 個市場日」拆成兩層,市場日以**參數**注入,所以單元測試用固定日期、零筆資料庫。順帶修掉一個實作陷阱:名目日早於日曆第一天時,照字面套用順延會把上個月的結算日推成日曆開頭那天,憑空造出一個排除日。
  - **`compute/futures_volume_battery.py`**:§1–§3 的可執行版本,`mode=ro` 連線、**不寫任何資料表**,只輸出 JSON 與 `radar futures-volume-battery --as-of ... --out ...`。裁決分三種結局(上線 / 冗餘 / 檢定力不足),另有安慰劑抽不滿時的「不可評估」。
- [x] pipeline pytest **625 → 685 passed、282 subtests**(新增 `test_settlement_calendar.py`、`test_futures_volume_battery.py`)。五條否決各有一個真的把它點著的構造案例,三種裁決與檢定 B 的失敗各有一個;29 條變異測試(逐條破壞再確認有具名測試抓到)全部被抓到。
- ⚠️ **§1–§3 有四處文字容許一個以上的讀法,選了哪一個寫在 battery JSON 的 `choices` 欄位裡,不埋在程式碼**:(1) 檢定 A 數的是**成熟**成員(未成熟數另外列出);(2) R2b 對**每一個**契約-日都判,因為 §3.3 的安慰劑合格日定義為「R1–R5 全過而 flag=0」;(3) 安慰劑池把該股**自己的 F 日**也排掉,不只排它之後 5 天(同一檔股票兩個契約可能同日一個舉旗一個沒舉,而那天的往後窗口與旗標臂逐日相同);(4) 任一檔抽不滿 k 天時判**不可評估**,不用比較少的安慰劑日去湊 `h_P`。四條都朝「更難過關」的方向,但仍是選擇——**要改請走 v2,不要留在程式裡不講**。
- ⚠️ **§2 R5 記為盲點,不是一條有在跑的檢查**:`contract_multiplier` 只存「現在的」乘數,沒有歷史,所以窗口內乘數變更偵測不到。文件本來就這樣寫,battery JSON 的 `refusals.R5_multiplier_change` 逐字說明它是盲點而非計數。
- ⚠️ **§4 步驟 4 尚未執行**:回補完成之前不跑 battery,也還沒有 |F|、|F_only|、每個 seed 的 `h_F / h_P / σ_P` 可記。這一輪**沒有**產生任何裁決;§3.4 要求的紀錄(含日期數、列數、`as_of`、第幾次執行)等 battery 第一次真的跑過再補。

## 2026-09-16 完成標記命名 bug 修復、22:00 改純匯入消除 3× 重複運算、夜間閘門與告警門檻隨之調整、期貨存在性切片打通到 UI

- 🚨 **閘門上線後第一個真正跨午夜的夜晚,就撞出完成標記的命名 bug**。09-15 22:00 那輪在 `compute-branch-stats` 花了 74 分鐘,部署收尾落在 09-16 00:25。完成標記檔名是用**寫入當下**的日曆日期命名,於是變成 `radar-branch-round-2026-09-16.done`,而夜間作業找的是 `2026-09-15`——log 印出 `evening round marker: <missing>`,乖乖重算整整 106 分鐘。**這個失敗模式只在 22:00 那輪跨過午夜時才會出現,而那正是設這個標記的唯一理由**:夜間作業 00:05 起跑,天生就會撞上一輪還沒做完的晚班。簡單案例下它是對的,唯一設計來擋的那個案例反而壞掉。
- [x] **修法:標記的檔名與內容要用兩個不同的時鐘**。檔名在**該輪開始時**就記下資料日期(供夜間作業找得到),內容仍是完成的那一刻——先前把兩者混用同一個「現在」。
- 🔍 **3× 重複運算,量出來的不是猜的**。對同一 `as_of` grep 正式機 log,三輪 stock-stat 列數:17:40=1,137,430、22:00=1,137,535、00:05=1,137,535——**22:00 與 00:05 逐位元相同**,近五個日期、三次執行皆如此。17:40→22:00 六個穩定交易日的差量僅 +326／+48／+170／+119／+0／+105 筆(基期約 113.6 萬),約 **0.011%**。等於一天約 3.7 小時(74 分鐘 ×3)在 1775MB 的機器上算同一份東西;鎖占用實測約 17:42–20:00、22:00–00:25、00:05–02:12。
- [x] **決定:22:00 那輪改成純匯入**(`BRANCH_ROUND_MODE=import`,只有 22:00 那行 crontab 帶這個前綴,手動執行預設仍是全套鏈)。**17:40 保留全套**——它約 20:30 發布,使用者還醒著;22:00 過去要到 00:25 才發布,只為了 0.011% 的差異。**代價如實記下**:只在 22:00 匯入才進來的列,現在要等**隔天** 17:40 那輪才會反映進 `stock_stats`,不是當晚——這是取捨,使用者已核准。
- [x] **夜間閘門跟著改,否則整個省下來的成本會吐回去**。原本比對「完成標記時間戳」與「最新匯入 `run_at`」;22:00 改純匯入後,最新匯入(約 23:00)必然晚於 17:40 的標記(約 20:30),夜間作業會**每晚**誤判需要重算。改為只看**標記是否存在**——足夠了,因為標記只在 `deploy_data` 成功後才寫。
- [x] **兩處錯誤註解一併修正**:`safe-branch-stats.sh` 原本寫「帳本讀的是它剛更新的資料」——不是。`branch_point_in_time_persist.py` 與 `branch_stock_pctile_counts.py` 都**直接讀 `branch_trades`**,不讀任何 stats 表。這正是為什麼 22:00 不跑 compute,帳本仍看得到那一輪匯入的資料。
- [x] **告警門檻改成時鐘時間,不再是耗時長度**:夜間作業完成晚於 **03:30** 才告警(保護週六 05:00 的 weekly-backup 鎖),取代先前的「總耗時 > 180 分鐘」。理由:耗時門檻把「撞期」與「變貴」混為一談——09-15 那晚 start-to-done 138 分鐘裡有 28 分鐘在等 22:00 那輪的鎖,套用 180 分鐘規則會判「正常」,而它其實比數字看起來更靠近備份鎖。已觀測完工時間 02:12、02:23。
- 🚨 **本輪自己引入一個誤報,且曾被前一位驗證者誤記成本來就有**。`set +e` **不會**讓 ERR trap 失效(實測:`set +e; bash -c 'exit 75'; rc=$?` 仍觸發 trap;`if bash -c 'exit 75'; then…else rc=$?; fi` 不會)。在 `f5bc663` 把 import 從「永遠回 0 的裸呼叫」改成會真的回傳 75/76 之前,ERR trap 沒東西可抓;之後每次 75 或 76 結尾都會**多送一則高優先級**「執行到第 N 行失敗」,疊在設計好的分級通知之上,抹掉 75-普通／76-高優先級的區別,等於在訓練 operator 忽略這個頻道。改用 `safe-branch-stats.sh` 裡 `run_step` 已經在用的 `if` 形式修掉。**`warrant-backfill.sh` 用同一種裸呼叫、有相同的假警報,這次刻意不動,是已知的既存問題,不是新發現。**
- [x] **正式機 crontab 已套用(使用者明示核准,備份 `~/crontab-backup-20260916-*.txt`,改動前後各 56 行)**:`5 0 * * *` → `5 0 * * 2-6`(消掉週日／週一 00:05 找不到前一交易日、整輪空跑約 110 分鐘的浪費);`0 22 * * 1-5` 加上 `BRANCH_ROUND_MODE=import` 前綴(17:40 那行未動)。**09-17 起生效**:17:40 全套、22:00 純匯入、00:05 夜間作業因此跳過那 74 分鐘 compute。
- [x] **期貨(futures)存在性切片打通到 UI,首份真實資料已進來**:09-16 21:20 首次匯入,`futures 2026-09-15: contracts=320 rows=1969`(feed 2,353 筆中 65 筆非個股期別已過濾)`sessions=一般/盤後`;320 檔中 301 檔 join 到 `type='stock'`、252 檔 distinct 個股。匯出 JSON 驗證三種狀態皆活著:2330 帶 `CDF`、`QFF`;00400A 帶空陣列 `"contracts": []`——「查過、確實沒有」而非缺鍵。**尚未完成,如實記下**:成交量異常排名(需 60 個交易日歷史 ＋ 四條事前寫死的拒絕準則)與首頁分頁——使用者三項原始需求交了一項,兩項未動。
- [x] pipeline pytest **615 → 625 passed、258 subtests**。
- ⚠️ **仍未查**:`compute-branch-stats` 的 per-stock 迴圈(約 74 分鐘)仍未剖析過。讀專用測量工具與等窗驅動器都已就緒,但乾淨窗口稀缺正是 3× 重複運算造成的,故意先擱著;等一天一次 compute 真正上線,約 23:30 夜間作業之後應該會有安靜窗。**新看到的**:per-step 計時顯示 `export-json` 佔 1033 秒(17.2 分鐘),是第二大步驟,從未被檢視過。
- ⚠️ **17:40 vs 22:00 的覆蓋率比較,現在可以直接讀,不必再推論**:兩輪都會各自寫一列 `branch_coverage`,直接比對那一對列即可。

## 2026-09-15 分點夜間作業重複發布根因查明、兩道發布閘門補上、狀態詞彙新增 `incomplete`、兩處先前數字更正

- 🔍 **夜間作業耗時「93 → 138 分鐘」的成長,大半不是變慢,是等鎖**。start-to-done 內含鎖等待;扣掉之後七晚為 91.1／90.9／92.0／94.0／93.7／107.1／110.2 分鐘。138 分那一晚是 110 分鐘工作 ＋ 27.9 分鐘等待。
- 🔍 **它在等什麼**:22:00 那輪 daily-branches 還在跑——匯入 22:54 收尾(分點匯入記了 `status='error'`),接著 `compute-branch-stats` 跑約 98 分鐘,到約 00:32 才部署。夜間作業 00:05 啟動、等鎖 1671 秒,然後**重新算了一次一模一樣的資料**,02:23 再部署一次。同一份資料兩次全套發布,重複約 110 分鐘運算。
- [x] **逐步計時補上,才量得出上面兩條**:`compute-branch-stats` ~98 分鐘;`branch-stock-pctile-counts` 641.5 秒(window=490d,pairs=1,329,077);`branch-point-in-time-persist` 95.6 秒。視窗掃描器合計約占整份工作的 **11%**;per-stock 迴圈才是大頭,**尚未查過**(`6907eff`)。
- 🚨 **更正一:權證列占掃描視窗比例先前寫成 31%,是錯的**。那次量到的是 90 天窗口;掃描器實際使用 **490 天**,權證列為 1,730,471 / 27,628,743 = **6.3%**。原始估計(31%)其實量錯了窗口大小才顯得高,而「更正」成 31% 這個動作本身才是本輪要修的錯——記在這裡當提醒:改窗口口徑要先核對是哪個查詢在用哪個常數。
- 🚨 **更正二:先前回報的兩起「死掉的來源仍記 `status='ok'`」生產事故,查證後都解除**(2026-08-14 rows=0 ok;2026-08-17 rows=2031 ok)。兩天都是分點 feed 正式發布**之前**的早期執行,且當天收尾都健康(distinct 1964、1970 檔)。那個洞在設計上確實存在,但從未真正發生過。**特別記下**:這是在論證「累加式逐日設計」為什麼必要時舉的例子,例子本身是假的,但論證要保護的那種錯是真的——下次舉證要先查發布時序,不能只看某一列的 `status`。
- [x] **發布閘門(a):`daily-branches.sh` 不再吞掉 `status='error'`**。`_run` 不拋錯,過去 import 記了 `status='error'` 整條 compute／export／deploy 鏈仍照跑——09-14 22:54 那次錯誤匯入就是這樣被發布出去的。CLI 現在依結果分四級(0 全過 / 75 個別目標失敗 / 76 本輪全數落空 / 其餘 unfit),shell 只依 exit code 分支;明寫 `set +e`,因為 `set -e` 對「75 個目標失敗」與「1 個目標失敗」一視同仁地整條中止,會製造這次要避免的 withhold-everything 行為(`f5bc663`)。
- [x] **發布閘門(b):夜間作業的略過條件改為雙重確認**。原本只看「import 那列狀態 ok」就判定可以略過——但 import 可以記 `ok`、`compute-branch-stats` 隨後 OOM,什麼都沒算也沒部署,夜間作業卻因為看到 `ok` 就跳過,等於在最需要它補救的那一晚拆了備援。現在要求「fit 的 import 列」**且**「deploy 之後才寫、時間戳晚於該 import `run_at`」的完成標記同時成立才略過(`6907eff`)。
- [x] **`import_logs.status` 新增 `incomplete`,兩個生產者、兩種意思,分開推導**:分點匯入指「個別目標有缺但涵蓋率落在可接受帶內」;權證爬蟲指「乾淨可續跑的分段停止」。修之前每次乾淨的權證分段停止都被記成 `error`,正式機曾連續三次假性失敗,這個訊號等於 100% 雜訊(`c1aafe4`、`8fe4c36`、`9d6e3bc`)。
- [x] **決定發布與否的統計量**:每個交易日、有分點資料的 distinct 股票數,**join `stocks.type='stock'`**——這個 join 是必要的,權證列與一般股票共用 `branch_trades_raw`,漏掉會讓某個回補過的日期多出約 18,000 個 distinct id,錯把回補當成覆蓋率良好。連續 26 個交易日落在 1949–1973,約 1.2% 的波動帶。
- [x] **閘門設計**:`expected` 取這一輪匯入請求的股票目標數,**在附加權證目標之前**就先記下;`ratio = min(1.0, coverage/expected)`。ratio < 0.5 判 unfit、**不發布**;0.5–0.8 只告警不擋;`failed > 0` 但 ratio 夠高判 `incomplete`,仍發布。**兩個門檻不同高是因為代價不對稱**:誤擋一次等於少發布一整天約 1,988 檔的分點訊號,誤放一次(80% 覆蓋率)等於當晚少 20% 股票的訊號——擋的門檻刻意訂低,只讓「死掉的 feed」摸得到;告警門檻可以照觀測資料調,因為調錯的代價只是一則通知。舊版本用「比對過去 60 天歷史」判斷是否異常,已捨棄——匯入器自己建目標清單,分母本來就精確已知,這條路徑消掉了 regime-shift、冷啟動與追平期三種問題。
- futures(期貨)功能本輪出貨單股存在性的 cron 與 export、個股頁指標(`f71deed`、`3d4cad5`);export 刻意不帶 ratio／average／rank／z 任何一個鍵,有測試守著這一點。剩餘切片(250 日回補、四條事前寫死的拒絕準則、成交量異常排名、首頁分頁)未動。
- [x] pipeline pytest **559 → 613 passed、246 subtests**。
- ⚠️ **下一輪待辦(已議定,尚未做)**:`compute-branch-stats` 內約 98 分鐘的 per-stock 迴圈完全沒剖析過,是目前夜間成本的主體(視窗掃描器僅占約 11%)。先 cProfile 20 檔股票,回報 SQL/Python 時間占比與前三名函式,**在提出任何改寫之前**;目前它一晚跑兩次(見上第一條),任何修法成本都要乘二。
- ⚠️ **待人工決定的 cron 問題,不是 bug**:00:05 夜間作業與 22:00 那輪的重疊應框成「把約 30 分鐘空等要回來」——閘門修好後撞期代價已從「重複發布」降為「單純空等」。連帶待辦:`5 0 * * *` → `5 0 * * 2-6`,消掉週日／週一 00:05 找不到前一交易日時的整輪約 110 分鐘重跑,一週省兩次。
- ⚠️ **已定案但尚未實作的 tripwire**:夜間作業完成標記晚於 03:30 即告警(保護對象是週六 05:00 的 weekly-backup 鎖)。先前「總耗時 > 180 分鐘」的版本已否決,因為那把「撞期」與「變貴」混在同一個訊號裡。
- ⚠️ **未測、也答不出來**:17:40 那輪是否達到與 22:00 那輪相同的 distinct 股票覆蓋率——兩輪 upsert 同一批 key,事後無法回推分辨。閘門設計在兩種答案下都安全,不影響本輪結論。
- ⚠️ **視窗掃描器改寫本輪刻意不做**,占比約 11%,留一個書籤:等 per-stock 迴圈修好、夜間作業真的降到約 30 分鐘,屆時 12 分鐘會變成約 40%,再重新評估值不值得動。
