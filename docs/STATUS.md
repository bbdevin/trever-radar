...
4a. **盤中訊號雷達 + 分點追蹤視角**(`docs/24`,2026-07-11 使用者指定排入):**Part B 分點追蹤視角**——/branch 排行榜合併入口,點常見分點看近 1/5/10/20/自訂日淨買超最多股票(export 每分點近 120 日精簡日明細、前端任意區間加總),**VPS 分點回灌完成後即可做**(B1 export → B2 前端);**Part A 盤中雷達**——Fugle + 本機 worker + Supabase 中繼,規則 I-1 大單/I-2 爆量/I-3 急拉/I-4 Armed 發動,I0 PoC 待使用者申請 Fugle key 即可先行,I1-I3 接 Armed A1 之後(I-4 依賴 Armed 名單)。
