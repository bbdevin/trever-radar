...
4b. **全市場擴容**(`docs/26`,2026-07-12 使用者定案「有幾檔抓幾檔」,推翻原 P3 不建議取捨):WP-M1 個股 JSON 池全市場(可立即)→ WP-M2 每日分點全市場一輪制(finalize 上傳後切換)→ **WP-M3 branch_hist.db 拆分+常駐 VPS+R2 週快照(高風險,待使用者批准 Planner 建議方案)** → WP-M4 VPS 全市場歷史 march-back(~11-12 天)。DB 紅線與 Actions 額度約束見 docs/26 §2。
