...
- 2026-07-10 13 項選股策略與獨立榜單重構:`indicators.py` 及 `scores.py` 實作涵蓋技術與籌碼（如「漲停二次發動」、「法人連買突破」、「均線糾結突破」等）共 13 種量化策略；前端首頁「策略」頁籤內，新增了可動態切換 15 種不同策略條件（包含舊有綜合嚴謹/寬鬆策略）的選單，並移除個別策略按鈕上的雜訊數字，介面大幅升級。
