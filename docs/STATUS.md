...
1. **權證主檔匯入**(標的對應/履約價/到期日)→ 權證異動分數的前提
2. **還原價**(除權息調整)→ 技術指標正確性的前提
3. **技術指標**(MA/RSI/KD/MACD/20日高/箱型)+ golden-file 測試
4. **評分模組**(04 規格;V1-Free 權重:權證30/技術30/法人融資25/題材15)→ 觀察清單 + 理由文字
5. 題材標籤(人工維護表)+ 題材熱度
6. 探索頁、自選股、訊號歷史回填
7. `deep-backfill --all` 全市場深歷史(使用者本機或雲端跑一晚)
8. 分點功能(等付費 FinMind 贊助決定,規格已備於 04/09)
9. V2 盤中(Fugle + 本機 worker)

## 已知債務 / 注意

- 個股 JSON 一檔約 0.5MB(全歷史);擴到數百檔時改「預設 5 年 + 按需載入」
- 權證 warrant_daily 已 1,000 萬列/年增速;彙總表(warrant_stock_daily)尚未建,評分前要做
