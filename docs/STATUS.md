...
| 公開狀態 | **已鎖站(私人測試版)**:2026-08-19 起門禁 = 站內 Google 登入 + `/data` Worker 驗 JWT/核准狀態。Cloudflare Access 為過渡雙鎖,**待使用者確認 Worker 401 後於 Zero Trust 關閉**。noindex + robots.txt 保留。 |
...
- 2026-08-19 **WP-B7 Worker JWT(Access 待關)**:`/data` Worker 驗 Bearer JWT + `app_profiles.approved`,或 `X-Radar-Service-Key`;前端 `dataFetch` 帶 token;盤中 worker 改帶 service key。Access Application 須等 VPS deploy 驗 401 後由使用者關閉。
