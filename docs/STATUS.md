...
5b. **首頁掃讀體驗+個股頁資訊架構統一**(`docs/28`,2026-07-12 規劃定案、未實作,純前端+輕 export 隨時可開工):WP-H2 語意色彩層次(理由 pills 家族色:籌碼青/技術藍/權證琥珀/風險紅,只用既有 token)→ WP-H1 榜單依題材分組(section+sticky+資金流熱度排序)→ WP-H4 移除個股頁分點 tab、K 線下方分點區為單一真相(A 案)→ WP-H3 卡片走勢改當日分時(**A 案已於 2026-07-12 批准**:Fugle key 進 Actions secret `RADAR_FUGLE_TOKEN`,實作時使用者親自設 secret;B 案降為缺資料 fallback)→ WP-H5 籌碼K線圖+分點明細手機版(2026-07-12 追加:手機子 pane 擇一顯示、vertTouchDrag 還給頁面捲動、買賣超改 segmented tabs、勾選浮動回饋 chip;桌機逐位元不變)。**全部規劃定案,等使用者喊開工**。
