...
## 2026-09-14 個股頁分頁對調、「籌碼不確實」查證為資料屬實（文案誤導）、權證回補開跑

- [x] **個股頁 `技術` 與 `資券` 對調**(使用者指定):順序只有一份陣列(`web/app/stock/page.tsx`),一處改完;`?tab=margin` 的外部連結(`MarginUsageRank.tsx`)照常運作,查詢鍵未變。順帶補上 `?tab=tech` —— `margin`／`holders`／`basic` 都接受網址參數,只有它沒有,那不是取捨是少寫一條。
- 🔍 **使用者回報「有些股票籌碼買賣超不確實,明明要抓每日十幾個分點,有時只列出前五六名」——查證結果:資料屬實,不是漏抓。**
  - **直接比對來源頁**(唯讀,五組(股票,日期)):存下來的筆數與來源頁**完全相同** —— 5 對 5、30 對 30、3 對 3、26 對 26、30 對 30。parser 也沒有截斷(`fubon.py::fetch_branch_trades` 是 `_ROW.findall(html)` 全收,無切片、無上限,只用 `key` 防禦性去重)。
  - **筆數與成交量同步**:5276 在成交 68,043 張那天 30 筆、50,654 張 30 筆、7,052 張 13 筆、2,050 張 6 筆、**1,023 張 5 筆**、2,000 張 3 筆。
  - **不集中於特定日期**:近 20 個交易日,每日 `rows<=8` 的股票數穩定在 **48–72**,`rows>=30` 穩定在 1,467–1,561。抓取失敗會表現為某幾天暴增,實測沒有。
  - **全體分布**(近 20 交易日、只算普通股):30 筆 **30,186** 個(股票,日期)是壓倒性眾數(15 買 + 15 賣),1–29 筆合計約 9,300,31–33 筆 93(同分點兩側皆入榜)。
  - [x] **真正的缺陷在文案,已修**:`BranchFlowSection` 兩處寫「前 15 大買賣超**裁剪版**」。「裁剪」宣稱的是「有東西被砍掉」——熱絡日為真(看不到第 16 名後),冷清日為假(沒被砍,當天就只有 5 家)。**兩種情況在表上長得一樣、意思相反,而我們只解釋了其中一種。** 改為明講「15 是上限不是配額」並同時說明兩種情況,附實測數字。`BranchPctilePanel:135` 本來就寫對了,本次是把個股頁籌碼區對齊它。
- 🔍 **`deep-backfill` 修復(`76466eb`)是活的,但從未執行**:容器 bind-mount `pipeline`,`DEEP_HISTORY_BEFORE = "2026-01-01"` 確實在跑的那份程式裡;log 仍是 `1097 fetched, 1317 already deep` 的原因是 **`data-backfill.sh` 01:10 用 `flock -n`,而夜間作業 00:05 起握鎖到約 01:40,它自 09-09 起每晚被擋**。正是 Fable 09-08 指出並接受的取捨。**實測獎勵大小**:`MIN(date)` 分布為 <2010 **1,317** 檔、2010–2025 **1,123** 檔、≥2026 **56** 檔 —— 修好後每晚成本由約 1,097 次抓取(約 119 分鐘 sleep)降為 **56 次、約 6 分鐘**,讓路的理由隨之消失,排程是否調整已交 Fable 5.1。
- [x] **權證全市場回補開跑**(使用者 09-08 授權「找時間跑」):`warrant-driver.sh` 長期迴圈,深度預設 90(目標 120)。09-04 那份 state(2,020 筆全 `ok`)直接沿用。驅動器本身不做判斷,要不要跑／跑多久全由 `warrant-backfill.sh` 的時間守衛決定;連續 3 次真失敗才停,單次鏡像抖動不殺掉一個要跑六週的工作。
- **五天運作實況**:夜間作業 09-11～09-14 每晚 `start` → `done`,`pit=ok`／`pair_pctile=ok`;`branch_pit_stats` 累積 5 個 as_of(09-04、09-08、09-09、09-10、09-11);時長 93／97／96／93／**107** 分鐘(有上升趨勢,列入觀察);磁碟 6.4 G。

## 2026-09-08 方向 battery 首次在修好的資料上執行：面板留下、往後看撤下、保留名稱不釋出（Fable 5.1 決策）

- **前提兩件都到齊才量**:`adj_factor` 全市場修復完成(09-04,2,413 檔、0 失敗);490 日分點回補完成(09-07,`oldest_filled=2024-08-28`、filled=490 / thin=0)。先前那些 obs/exp 2.94 / 2.56 / 2.34 是在不完整資料上算的,**作廢**。
- **執行**:`radar branch-window-direction-battery --as-of 2026-09-08 --window-days 490 --seed 20260908`,唯讀,13 分鐘,rc=0。切分:形成 2024-09-02～2025-09-03(245 日)、評估 2025-09-04～2026-09-08(245 日)。

  ```
  backward: flagged=197 stocks=177 activity=193 survivors=155 exceeds_both=89/155
            obs_exp=2.395859  lag=2.152416  placebo=0.913722  re_flag=4/197
  forward:  flagged=31  stocks=30  activity=26  survivors=18  exceeds_both=9/18
            obs_exp=2.151917  lag=1.917883  placebo=0.936790  re_flag=0/31
  ```

- [x] **① 已上線的證據面板留下。** 事前寫死的撤回門檻是 backward obs/exp < 1.5,讀數 2.396。抽樣檢查:89 觀察對 37.15 期望、n=155,固定 exp 取觀察數 2σ 下界(89−2×6.2≈77)仍得 obs/exp ≈ 2.06,**沒有合理重抽樣會跌破 1.5**。自由度檢查:seed 由 20260904 改為 20260908 不可能影響 backward,因為準則④無安慰劑項;協定常數(10/0.7/245/±25%)未動。佐證(非準則):lag 2.396→2.152(保留 0.90,「自身價格衝擊」的說法會讓它塌掉);流量配對安慰劑 0.91(等同隨機)——**比舊的 episode 配對安慰劑 1.12–1.24 更低,先前「舊安慰劑漏掉規模干擾」的疑慮是真的,但方向相反**。
- 🚨 **更正一個本專案自 09-03 起反覆引用的誤讀:`re_flag` 不是「傾向只有 2% 會持續」。** 它量的是**嚴格旗標**(≥10 次已知/側 **且** 兩側 ≥70%)的再現率,把「活動量」和「比率持續性」綁在一起——一個分點在評估半段交易少一點就不可能再滿足 ≥10 次,與它的低買高賣傾向是否持續無關。回答「紀錄有沒有帶到未來」的統計量是**存活者中兩側都勝過自身股票基準者 57.4%,對照隨機/安慰劑的 22–24%**。**2% 禁止徽章與「總是」;57% vs 24% 授權呈現計數。** 面板現在的形狀正好編碼這個分界,結論不變,但先前對「為什麼對」的解釋是錯的。
- [x] **② 往後看依自身準則撤下,並記明那是什麼。** `forward_vs_flow_matched_placebo`:差距 0.2778 對 2σ 的 0.3065,即 **1.81σ ——「檢定力不足的無結果」,不是否證**。n=18 時安慰劑少中一次(3/18 而非 4/18)即翻成 KEEP;在觀察到的比率下需每側 ≥22 個存活者才有把握。**正因為靠單次抽樣就能翻盤,事前寫死的答案必須算數:反覆換 seed 直到過關,正是 seed 參數要暴露的失敗模式,不是要促成的。** 保留於 battery JSON 內部(準則③依賴它);**取消** 09-04 規劃的 additive 快照欄位與每側第二條長條圖。`docs/04` line 66 改記「2026-09-08 於 n=18 未過,未實作」。再資格化條件:未來某次執行 forward 兩臂存活者皆 ≥ `LOW_SAMPLE_SURVIVORS`(30),且三條 forward 準則在多 seed 執行的**每一個** seed 下皆過。
- [x] **③ 面板文案不動。** 加上「已驗證 2.4×」等於變相徽章,會誘導把計數讀成分數;09-03 排除它的理由成立,新數字不改變它。「不代表之後會延續」講的是個別 pair,正是 re_flag 說的事。
- 🚨 **④ 準則本身有漏洞,已補(僅對未來執行生效)。** 09-04 的文字寫「塌向**它的安慰劑**」,但程式(`branch_window_direction_battery.py:655`)只檢查絕對水準——**`1.6` 對安慰劑 `1.4` 今天會被判 KEEP**。今天兩種讀法一致(2.40 vs 0.91,約 6.8σ)故未影響決策。新增兩條事前寫死的 backward 準則:**安慰劑相對**(exceeds-both 落在其流量配對安慰劑 2σ 內,或 backward 安慰劑 obs/exp > 1.5 → 撤)與 **lag 保留比 < 0.5 → 撤**(今日 0.90)。門檻維持 1.5:它是在安慰劑估為 1.12–1.24 時訂的,實測 0.91 使它比原意更寬,但**看到讀數之後才調門檻會像在調參**;安慰劑相對準則是尺度無關的,已涵蓋該疑慮。「存活者 < 100 或 obs/exp < 2.0」列為**人工複查觸發,非自動撤下**。`re_flag` 仍**不是**準則。
- [x] **⑤「關鍵分點」保留名稱不釋出。** 那個名字指「某分點在**這檔股票**上反覆低買高賣」,是**身分宣稱**;battery 說群體層級傾向持續(2.4×),而身分只有 2% 再現。**產品中沒有任何物件具備那個名字所斷言的性質**,面板也刻意不製造一個。`test_label_honesty.py` 的守衛留著,註解改為「battery 已過、面板留下,但名稱仍不啟用,因為面板不做那個名字所做的宣稱」。釋出條件:出現一個有自身持續性量測的構造,標準比照隔日沖徽章(`docs/13` §8.1),且**再現率門檻須在量測之前寫死**。
- ⚠️ **重測政策**:①`daily_prices` 缺口修復**執行之後**立刻重跑(那 15,150 個股票×日落在評估半段內;機制是 `_close_range_percentile` 回 `missing_close_in_20d_window` → `unknown` → 該 pair 與該股基準**同時**被排除,是樣本流失不是方向性偏誤,故不推翻今日決策);②其後每約 60 個市場日(下次 as_of 約 2026-12-01);③`DEFINITIONS_VERSION`、`QUAL_PCT`／`PRICE_WINDOW_DAYS`／旗標常數、adj_factor 重算、回補深度變動時各一次。**每次以 3 個 seed 在同一 as_of 執行。** battery 是手動工具,而本專案有「手動工具靜默沒在跑」的前科(09-03 稽核),下次到期日寫進 `handoff.md`。
- ⚠️ **已知未涵蓋**:匯出端顯示的是每側 ≥5 次(`json_export.py:62`),而 battery 驗證的是 ≥10/0.7(09-03 驗證過 ≥8)。**5–9 這一段沒有樣本外讀數。** 不憑猜測移動門檻(分母有顯示,「5/5」看得出很薄,與文案一致),列入重測清單:加 `--flag-min-known` 參數,同時讀 5／8／10。另:margin 排序取前 10 是一種軟排名,已接受(不顯示名次)。
- ⚠️ **產出檔第一次遺失**:`--out /home/huang/battery-2026-09-08.json` 寫進的是**容器內**未掛載路徑,`--rm` 後消失。數字自 stdout 留存,判決不受影響;重跑改用掛載路徑(`data/...` 於容器內解析為 `/app/pipeline/data/`)。**同類錯誤同日出現兩次**(權證 state 檔亦落在 `pipeline/data/` 而非註解宣稱的 `data/`),已列為待修。

## 2026-09-04 「關鍵分點」正名為「追蹤分點」，關鍵分點成為保留字（使用者決策，已上線）

- **起因**:使用者指出「有些現有的關鍵應該命名不叫關鍵才對,那是我自己看的觀察券商」。查證屬實,且比使用者說的更嚴重——`K1_KEY_BUY` 的觸發條件裡**沒有價格、沒有分位、沒有低買高賣**。`pocket.py:342-351` 取 `tracked_branches` 聯集 `branch_rankings` 中 `rank_score >= 70` 且非隔日沖者;`tracked_buy_trigger` 只判「近 5 日淨買 ≥ 500 張或 ≥ 成交量 0.3%」。純粹是身分與規模,和「在相對低點買、相對高點賣」毫無關係。
- **兩個定義撞名**:專案裡「關鍵分點」有使用者給的明確定義——某分點在**特定個股**上反覆低買高賣,per-stock、價格分位,對應 `branch_stock_pctile_counts` 與 `BranchPctilePanel`。兩件無關的事共用一個名字,而其中一個從未量過那件事。
- **命名定案(使用者選定)`追蹤分點`**:沿用既有詞彙(`BranchTrackView.tsx:195,208` 的「尚無追蹤分點」、資料表 `tracked_branches`)。「觀察」被排除,頁面上已有觀察門檻／觀察池／僅供籌碼觀察,再多一個專有名詞會混。知情取捨:該集合並非全是手動追蹤,`rank_score >= 70` 那半段是系統自動入榜,名字略寬。
- **「關鍵分點」定為保留字,產品中任何地方都不使用(使用者選定)**:不掛到已上線的證據面板上。面板刻意不做二元斷言,因為實測標籤再現率僅 1.6~5.4%;且方向 battery 尚未在修好的價格上跑過,面板仍在可撤回清單上。**名字等 battery 判決。**
- [x] **實作**(`9a79fe7`):`key_buy_trigger`→`tracked_buy_trigger`、`KEY_*`→`TRACKED_*`、`key_keys`→`tracked_keys`、family `"KEY"`→`"TRACKED"`、code `K1_KEY_BUY`→`T1_TRACKED_BUY`、文案「關鍵分點同買」→「追蹤分點同買」,並在 trigger 上留言寫明**它不是**什麼。另修 `/branch` 排行卡上同一集合(`rank_score >= 70 || source === "manual"`)的「關鍵」徽章。
- [x] **雙讀過渡**:程式碼走 Pages(數分鐘)、JSON 走 VPS export(當天匯出輪),必然並存。`PocketBadges.META` 同掛 `T1_TRACKED_BUY` 與 legacy `K1_KEY_BUY`、皆指向「追蹤分點」;`ReasonPill` 接受兩種前綴;`types.ts` family union 保留 `"KEY"`。
- [x] **補一項執行者未涵蓋的過渡漏洞**(`998acaa`):個股頁是 `compact=false`、渲染的是伺服器產生的 `t.text`,tooltip 亦然,故舊 payload 會出現「徽章寫追蹤分點、展開卻寫關鍵分點同買」。加 `displayText()` 只對 legacy code 換開頭前綴,分點名稱原樣保留;與 META 的 K1 條目同時可刪。
- [x] **迴歸守衛**:測試斷言輸出 tag 的 `code` 與 `text` 中不得出現「關鍵」二字——這次的失敗模式就是有人把名字加回去。pytest **465 passed、100 subtests**;`tsc --noEmit` 乾淨。
...
- [x] **決定**：**往前看（事前 20 日）是「相對低點」的定義之記錄，且是唯一的篩選條件與目前唯一顯示的數字。** 往後看以獨立名稱「事後 20 日區間分位」被接受，additive 存於同一張快照表、面板上作為每一側的第二條 bar，**但必須先通過一套已提交版控、可重現、且跑在修好還原價上的樣本外測試**。**往後看永遠不會把任何分點帶進面板**（選擇權留在事前）。
- [x] **往前看勝出的理由不只是「它被驗證過」，還有偏誤方向**：某分點週一／三／五都買時，**週五的事前窗口已含它自己週一、三買出來的漲幅**，使它**更難**拿到低分位——自我汙染是**保守**方向，已驗證的 2.94× 沒有被分點自身行為灌水。
- [x] **往後看被更嚴格把關的真正理由（本輪最重要的發現，主協調先前未察）**：同股基準確實化解了「買到上漲股票就得分」這個天真反對意見，但有兩件事活在**分點層級**、基準吸收不了——
  - **①前 15 大切片本身製造價格衝擊**：每個被觀測的事件依定義都是「淨流量大到擠進前 15 大」的日子，集中買盤會在其後 1–5 日推動價格；**往後看時該衝擊落在窗內**，把事件收盤推向區間低檔。該股 pooled 基準吸收的是*平均*衝擊，吸收不了*該分點自己*的衝擊，而後者與其張數成正比。**既有安慰劑是以 episode 數配對、非張數配對，故從未測到這件事。**
  - **②自我汙染方向翻轉**：同一個週一／三／五的分點，**週一的事後窗口含它自己週三、五買出來的漲幅** → **更容易**得分。**往後看的偏誤方向，正好是會「證實」內線理論的方向。**
- [x] **仍接受往後看的理由**：往前看永遠無法檢驗使用者的內線理論——一個事前紀律完美的分點，可能只是在下跌趨勢中每次逢低承接。若這個理論值得做成面板，就值得被正確量測；而**張數配對安慰劑＋lag 測試**確實能檢驗上述衝擊混淆。若它沒過，那個結論本身也有價值，必須寫回 `docs/04`。
- [x] **還原價不破壞 point-in-time 承諾（技術論證）**：`adj_factor` 是**向後累乘**的，故窗口**之後**發生的除權息會讓窗內 20 個收盤同乘一個常數，而**常數在 min/max 分位中相消**。因此事前分位即使改用還原價，仍只是過去價格的函數。
- ⚠️ **順帶抓到兩個現成缺陷**：①`branch_point_in_time_persist._price_rows_for_stock` 讀的是**原始 `close`、完全沒乘 `adj_factor`**（`indicators.py`／`performance.py` 都有乘），所以連已修好的四檔也仍以未還原價算分位；②**先前那套樣本外驗證從未被提交成腳本**，只有結果留在本檔，撤回決策將無法重現——新的 battery **必須提交版控**。
- ⚠️ **事先寫死的撤回條件（往後看）**：協定同 2026-09-03（245/245 切分、各半段重建 episode、兩側各 n≥10、兩率 ≥0.7、一律對該股評估半段自身比率），跑在修好的還原價上、且 490 日回補完成後，另加 **①張數配對安慰劑**（同股、未標記、episode 數與事件日 `|pct|` 中位數各 ±25%）與 **②lag 測試**（改用次日還原收盤為參考價；內線買在消息前次日仍低分位，衝擊造成的則不會）。**任一成立即撤回**：與張數配對安慰劑差距在 2σ 內、或安慰劑本身 obs/exp > 1.5；lag 測試下 obs/exp 較未 lag 掉超過一半；或往後看 obs/exp < 2.0 而同批資料上往前看 ≥ 2.5。**再標記率不是撤回條件**（counts-not-badges 已吸收該發現）。
- ⚠️ **往前看也需要在還原價修好後補跑一次確認**：若 obs/exp 在還原價上**塌向安慰劑（< 1.5×）**，代表先前的驗證是未還原區間造成的假象，**應撤下面板**。Fable 明確要求把這個可能性寫成活的風險，而不是假設它不會發生。
- ⚠️ **中間窗（前 19 ＋ 後 20，「買在 40 日谷底」）已被否決**：它是內線故事最乾淨的單一表述，但那是**第三個**定義、零驗證，且會讓一個數字默默混合兩種機制。若兩個方向都過關，才是它的後續。
...
- [x] **可見門檻遠低於「前 15 大」聽起來的嚴苛度**（2026-09-02 當日實測）：買方真的滿 15 家的個股 1,363 檔，其第 15 名淨買張數為 p10 **1 張**、**p50 4 張**、p90 28 張、p99 198 張、max 450 張；賣方 1,324 檔，p10 1／p50 5／p90 30／max 911。**中位數個股只要淨買約 4 張（4,000 股）就會被記錄。**
- [x] **三成個股根本沒被截切**：1,942 檔有淨買方的個股中，**579 檔（30%）買方不滿 15 家**——那些股票看到的就是全部買方。
- [x] **每日涵蓋 1,949–1,968 檔**，即幾乎所有有交易的個股；`branch_trades` 曾出現的個股共 1,986（active 普通股 2,047）。每日 30 列（15 買+15 賣）者 1,466 檔，其餘為未滿上限。
- [x] **據此更正 `docs/13` §6 兩條「誠實限制」**：①「只有前 15 大」補上實測門檻，並改述為「計數是活動量的**下限**」，真正看不見的是**熱門股上的極小部位**，而非一般的小額分點；②「只涵蓋每日評分池 80 檔」**已過時作廢**——`daily-branches.sh` 自 2026-08-31 起為 `--top 0`，冷門股不再沒有分點史。
- ⚠️ **同時更正我方稍早的說法**：先前在關鍵分點的 UI 規格中寫「一個每天小量買的分點可能完全看不見」，該說法只對熱門股的極小部位成立，對多數個股是錯的；已把實測數字送交 UI 實作者改寫文案。
...
- [x] **原料早已存在**：E2 shadow 的 `low_buy`（買進 episode 且 20 日收盤分位 ≤ `LOW_BUY_MAX_PCTILE` 0.40）與 `high_sell`（賣出且 ≥ `HIGH_SELL_MIN_PCTILE` 0.60）正是這個概念，且本來就在**分點×個股**層級計算。缺的只是判定門檻、當下快照的持久化，以及 UI。
- [x] **關鍵前提：兩側基準率不對稱。** 全市場 pooled 低買 **53.35%**、高賣 **35.35%**（20 日收盤分位在本市場偏底部）。**所以「兩邊都 ≥0.5」的規則在買側幾乎免費，實質是只篩賣側的規則披著雙邊外衣**；`≥0.5` 各格對每檔股票自身 null 的 obs/exp 僅 1.07–1.45×，屬選樣噪音，應直接丟棄。
- [x] **樣本數不是瓶頸**：490 交易日窗口下，即使要求兩側各 10 筆已知分位 episode，仍留 **85,960 對、涵蓋 1,929 檔**（97% 的股票 universe）。unknown 分位佔比僅 6.4–6.5%，全程分開計數。
- [x] **樣本內（490 日）以每檔股票自身 pooled 率為 null**：`n≥8 且兩邊 ≥0.7` → **691 對／509 檔／5.08×**；`n≥10 且 ≥0.7` → **361 對／291 檔／7.05×**，且該 361 對**全部 361 對**在兩側都勝過自己那檔股票的基準。分點層級異質性亦可測：pair 低買數對自身股票 pooled 率的 χ²/df 在 n≥10 為 **1.71**（賣側 1.45）。
- [x] **樣本外時間切分（決定性測試）**：形成 2024-08-27→2025-08-28（245 日）／評估 2025-08-29→2026-09-02（245 日），episode 於各半段獨立重建。全市場基準有漂移（低買 0.5177→0.5433、高賣 0.3683→0.3444），故所有比較均對**該股評估半段自身**的比率。存活率 74–86%（標記的分點一年後仍可測量）。**兩側都勝過自身股票**：`n≥10/0.7` **67.3%**、`n≥8/0.7` 62.1%、`n≥10/0.6` 56.1%；obs/exp 分別為 **2.94×／2.56×／2.34×**；中位超額買側 +13.9～+27.1pp、賣側 +10.6～+17.9pp。
- [x] **規模配對安慰劑（最可能推翻此假設的對照）—— 通過**。在同一檔股票取形成半段兩側樣本數 ±25% 內、未被標記的分點：obs/exp 僅 **1.12–1.24×**（貼著機率），中位超額 +0.4～+2.0pp。「兩側都勝」flagged 67.3% vs placebo 29.5%（z≈3.9）、56.1% vs 29.0%（z≈6.6）。**規則篩到的不是曝光度或部位規模。**
- ⚠️ **但標籤本身不可重現**：形成半段被標記者，在評估半段用同一規則**再次被標記**的比率為 `n≥10/0.7` **1.6%**、`n≥8/0.7` **3.6%**、`n≥10/0.6` **5.4%**（安慰劑 0–0.6%，相對 9× 但絕對值極低）。**這直接牴觸「總是低買高賣」的定義**——傾向為真，「總是」不成立；今年合格的那批，明年幾乎是完全不同的一群。
- ⚠️ **設計含意（待人類定奪，尚未實作）**：不應做成暗示永久性的二元「關鍵分點」徽章——資料不支持該穩定性。可支持的是**呈現證據與分母**（例：「買 33 次其中 28 次在低四成；賣 20 次其中 14 次在高四成；該股基準 57.2%／31.9%」），措辭為「歷史上在這檔股票買點偏低、賣點偏高」，與隔日沖徽章同一標準。**全部是進出場價格分位，不是損益**；`docs/37` 禁止買賣配對與獲利歸因，故不得推論賺賠。
- ⚠️ **量測需在回補完成後重做**：評估半段的已知 episode 比形成半段**多 60%**（346 萬 vs 216 萬），因 490 日回補尚未填完舊資料，故形成半段門檻偏保守、flag 數（64／140／368）為低估。其餘限制：單一切分、單一市場區間；`n≥10/0.7` 僅 55 個存活樣本，其點估計不可讀到兩位數；`branch_trades` 為前 15 大切片，故「兩側各 10 筆」本身先是集中度篩選（安慰劑對照已中和比較，但未中和入選資格）。
...
### 第二軸（同日）：前端型別宣告了、卻沒有任何元件渲染的欄位

- **方法**：抽出 `web/lib/types.ts` 所有欄位名，掃全部 `web/**/*.tsx|ts`（排除 `types.ts` 自身、`node_modules`、`.next`、`.agents`）看是否被引用。**這一軸大致乾淨**——205 個宣告欄位中約 43 個未被引用，逐一查證後多數是解構賦值造成的假陽性，或刻意的決定，不是缺口。
- [x] **假陽性但值得記錄**：`insider_pct`／`pledged_shares`／`shares_at_election` 看似未渲染，實際上 `HoldersPanel.tsx` 有渲染董監持股，只是程式碼註解明寫「**D2 內部人％ UI 暫隱藏**」（`:53`）與「週表暫不顯示內部人％（**口徑待定**）」（`:426`）——**刻意延後，不是漏掉**。
- ⚠️ **唯一真發現：`summary_text` 是「還在生產、沒人消費」。** `json_export.py:1290` 的 `_build_summary_text()` 每次匯出都會執行、`types.ts:478` 仍宣告該欄位，但 `docs/07` §39 記載 **2026-08-31 已刻意移除日報框**（與市場卡、freshness 重複）。風險不在浪費，而在**下一個 agent 看到 payload 有欄位卻沒有 UI，可能會「修好」它**——把一個刻意移除的面板加回去。若要清理，請刪 exporter 端；**不要**重新加回 UI。
