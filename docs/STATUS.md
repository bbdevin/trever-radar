...
- [ ] **下一步是 §4 步驟 3–5,尚未動工**:唯讀的 `pipeline/radar/compute/next_day_surge_battery.py`(`LOW_SAMPLE_SURVIVORS` 從 `branch_window_direction_battery` import、`PLACEBO_SEEDS` 從 `futures_volume_battery` import,不複製第二份 30 與第二份 2σ)、跑一次、把 §3.5 的紀錄寫回本檔、**只有 PASS 才**加 `list_next_day_surge` 鍵。
