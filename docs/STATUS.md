...
- [x] **§4 步驟 5 的 export 切片落地(`7d8e198`),不擁有規則,只重用 battery 的**:`_oi_change` → `oi_change`、loader 轉 public,新增 `evaluate_contract_day_in_calendar` 把 window builder、純規則與 `oi_change` 併成單一進入點,`_assemble` 與匯出共用同一個呼叫點,不是兩份邏輯各自維護。`anomaly_facts` 把結果轉成 §1 五個整數鍵或 `None`——`None` 同時代表「被否決」與「規則跑過、沒有旗標」,和 `futures` 鍵本身的三態一致,不能印 0 或掰一句 not-published 去填缺值。Mutation 檢測 21 處存活 19,兩個倖存者等價(60 已被 §3.5 凍住、`FrozenConstantTests` 鎖住;匯出落後匯入日時的提前 return,與逐契約「今日無列」檢查等價)。
