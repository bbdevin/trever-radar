...
- [ ] **個股手機首屏 UI 校正（待視覺 QA）**：實作名稱與報價／漲跌同行、產業下方的活躍題材 2+N、Decision Header 的顯著分數與永遠可見首要判讀、一張低邊框行情 `dl`、單列可點入基本資料的概況，以及既有八個一級 tab 的 active 捲入。未改資料/API/export/globals/KChart；`design-qa.md` 尚待 375px 深淺色的實際視覺驗證，不能視為已驗收。
