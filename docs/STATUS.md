...
3. **B 方案 Phase 3—策略績效閉環**(`docs/20`):已新增策略績效報告產出器 `phase3-strategy-performance-report`（只讀 DB、輸出 markdown:樣本數/5-10-20 日勝率與平均/中位/最近區段）。下一步需在 VPS 上跑最新資料日產出正式報告，再由使用者決定 Active/Retired。
