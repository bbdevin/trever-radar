...
6a. **地緣券商+庫藏股分點+關鍵分點同買 → 口袋名單**(`docs/27`,2026-07-12 規劃定案):~~G0 資料 PoC~~ **2026-07-12 完成**;~~**G1 資料層 2026-08-20 完成**~~(`company_profiles`/`broker_branch_geo`/`python -m radar import-geo`,週一排程;庫藏股無 OpenAPI 延後)。**剩餘 G2–G4**(演算法+口袋 UI)。地緣涵蓋度在 7a 全市場每日池後才完整。
