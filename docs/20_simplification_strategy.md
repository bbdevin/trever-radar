...
| Phase 1 UI 刪減與合併 | ⏳ 待使用者另行確認實作 | 低風險,但仍須先看現有 diff |
| Phase 2 策略/分數解耦 | ⏳ 待確認 | 高風險資料語意變更;正式重算另設關卡 |
| Phase 3 策略績效閉環 | ⏳ 待確認 | 先做客觀報告,不由 agent 自動升級策略 |
...
**本階段定案**:保留 Supabase Auth / watchlist 作跨裝置個人化,但不擴張假性
會員資料閘門。需要真正私有時只能由使用者另案決定啟用 Cloudflare Access。
