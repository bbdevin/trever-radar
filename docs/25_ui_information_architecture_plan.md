...
## 15. 個股公司資訊／集團鑽取（2026-08-27，docs/37 B+D）

- 這是既有個股樞紐的 additive 資訊，不新增一級導航或另一個總覽頁。
- 名稱下方公司資料保留 compact 視圖與可鍵盤操作的 44px 展開控制；長地址 truncate 後可讀全文，缺欄位不猜測補值。
- `/group?id=` 是唯一允許的次要鑽取路由：由個股 badge 進入、集團成員可回個股，提供返回雷達、loading／不存在／缺報價的教育性空狀態。
- 集團頁不得把關係、價格或成員資格包裝為投資建議；只呈現版本化官方來源 mapping 與資料日。
