...
- 手機 375px 的名稱與報價／漲跌必須同行；身份列（代號／市場／產業）與最多兩個加 `+N` 的嚴格活躍題材置於下方。
- 首屏 DOM 與視覺順序固定為 header → Decision Header（分數與首要判讀可見，仍可展開／收合）→ 單一卡片行情摘要 `dl` → 單列公司概況（點入基本資料）→ 八個一級 tabs。不可回到六個獨立行情小卡或概況 2×2 grid。
