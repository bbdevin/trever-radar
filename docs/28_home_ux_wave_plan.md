...
## WP-H1 首頁榜單依題材分組(純前端,零管線改動) ✅ 2026-08-20

- **資料已齊**:radar.json `stocks[].themes`(股票卡題材標籤既有)+ `themes[]` 資金流榜(vs20 排序)。
- **互動**:榜單工具列加「排序:分數 | **題材**」切換(記 localStorage)。題材模式下:
  - 組標題橫跨整列(題材名 + 資金流 badge `vs20 +X%` + 檔數),依 vs20 降冪;卡片與分數榜同一套 grid(`md:2 / lg:3 / xl:4`)。桌機預設全開,手機前 3 組展開。
  - 組內卡片按原榜排序;**一檔多題材 → 只歸入當日最熱的那個題材**(避免重複卡);無題材 → 末尾「其他」組。
  - Section header sticky(手機捲動時知道在哪組);組可收合。
