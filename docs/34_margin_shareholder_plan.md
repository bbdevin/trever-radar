...
2. **檢視**：`張數比例`｜`持股人數`  
   ...
   - 人數：該門檻以上持有人數加總  
3. **圖**：依模式畫序列 + 可選股價灰線  
4. **表**（對齊籌碼類 App）：日期｜大戶持股(%)｜大戶增減(%)｜散戶持股(%)｜內部人持股(%)；紅漲綠跌＋+/-；散戶＝tier1–11（未滿400張）；內部人 Phase 後接（暫 —）  
...
| 資料 | 排程 | 範圍 | 備註 |
|---|---|---|---|
| 日 K | `daily-market` 14:10；**15:00 上櫃主補抓**；16:10／17:40／22:10 保底 | TWSE/TPEx **全市場單請求**（含 ETF） | 當日**無成交**個股官方報表本身無列 → DB／JSON 不會有該日 K。**上櫃 dailyQuotes 14:10 常尚未出表**（約 14:57 起才有完整表） |
