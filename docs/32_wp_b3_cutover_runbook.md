...
> **狀態:已於 2026-07-18 執行完成**(Step 5 repo-private 事後因 Actions Billing 反轉,repo 維持 public,
> 見 `docs/31` §12);本檔保留作歷史紀錄與回滾參考(回滾步驟 `docs/31` §8 於回滾窗內仍有效)。
> 本檔原是 `docs/31` §6 WP-B3 六個步驟展開成精確到指令/diff 的操作清單,
> 讓 cutover 當天變成照抄執行,不用臨場現想。前置條件:WP-B2 影子驗證連續 2–3 個交易日通過(驗收清單 `vps/README.md` §9)。
