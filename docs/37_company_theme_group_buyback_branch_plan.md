...
- **2026-08-28 IA-5c 呈現補強**：保留上述一級 tab 契約，名稱／報價區補開高低收、量額、資料日與可切入基本資料的 compact 公司概況；完整地址／股務代理／來源仍在基本資料面板。權證逐檔摘要新增資料日；個股權證分點改用 100 萬 index + per-stock shard，避免手機下載全市場大檔；500 萬的全市場探索契約、W5／首頁／Armed 門檻均不變。
