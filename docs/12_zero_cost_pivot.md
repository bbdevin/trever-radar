...
無付費 → 全市場分點資料無合法穩定免費來源(TWSE bsr 有 CAPTCHA,不做破解;FinMind 分點屬贊助方案)。因此:

- **V1-Free 評分改為**:權證 30% + 技術 30% + 法人/融資 25% + 題材 15% − 風險扣分。程式保留 branch 分數插槽(回傳 null 不計權重,權重自動歸一化)。
- 個股籌碼 K 線(09)的「分點足跡」部分延後至有預算月付 FinMind 贊助(約數百元)時啟用;個股頁先有 K 線 + 指標 + 法人/融資 + 權證四區。
- 04 的分點模型規格全部保留不刪——資料一接上即生效。
