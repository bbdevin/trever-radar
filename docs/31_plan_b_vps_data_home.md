...
- 2026-07-18(晚)**WP-B3 cutover 收尾驗證:Step 1-5 全落地,發現 Actions Billing 阻斷**:使用者回報 VPS/瀏覽器側完成(兩個 Worker deploy、intraday `.env`、repo 轉 private——`gh repo view` 確認 `visibility: PRIVATE`)。Agent Step 6 驗證:①紅線通過——未登入 curl `radar.techtrever.com/data/radar.json`、首頁、`trever-radar.pages.dev/data/radar.json` 全部 302;②5 支資料 workflow 於 07-17 之後零觸發(trigger cron 停用生效);③cutover commit 的 deploy 成功(純 build 1m4s)。**遺留:repo 轉 private 後 Actions 被帳戶帳務擋下**——其後 3 次 deploy 全部 3 秒 instant-fail 零 step,annotation「The job was not started because recent account payments have failed or your spending limit needs to be increased」;僅擋程式碼部署,資料鏈(VPS→Worker assets)不受影響。**同晚使用者定案:修 Billing 需綁付款方式、牴觸免綁卡原則,repo 改回 public(§6 WP-B3 Step 5 反轉,本檔 §2 根因 3「repo public 資料散布」已由 WP-B1 GitHub 零資料解決,不因 public 復發)**;改回後 deploy 恢復(1m7s 成功)、未登入三入口紅線重驗仍 302。其餘 Step 6 項目(登入後 freshness/全站功能、VPS cron log、盤中 worker 隔日 08:55 online)待使用者確認;全過後回滾窗兩週倒數(§8),接排 WP-B5。
- 2026-07-18 **WP-B5 文件大同步完成**(AGENTS.md 危險清單改寫、docs/08 §0 重寫為 VPS cron 表、DEPLOY.md、vps_backfill_plan.md Step 4e 作廢、STATUS.md、docs/32 狀態標頭)。
